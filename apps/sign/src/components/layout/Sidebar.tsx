...
const iconMap: Record<string, React.ReactNode> = {
  ...
  permissionDefaults: (
    <svg className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M9 3h6l6 3v6c0 5-3.5 8.5-9 10-5.5-1.5-9-5-9-10V6l6-3z" />
    </svg>
  ),
  storeAnalytics: (
    <svg className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
  ...
  billing: (
    <svg className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-2.21 0-4-1.79-4-4s1.79-4 4-4c1.657 0 3.076.81 3.828 2" />
    </svg>
  ),
  accountSettings: (
    <svg className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
...
function getIconKey(label: string): string {
  const mapping: Record<string, string> = {
    ...
    'nav.auditLog': 'auditLog',
    'nav.plans': 'plans',
    'nav.support': 'support',
    'nav.permissionDefaults': 'permissionDefaults',
    'nav.storeAnalytics': 'storeAnalytics',
        ...
        {visibleItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {!collapsed && item.label === 'nav.store' && (
                <span className="ltr:ml-auto rtl:mr-auto rounded-full bg-emerald-500 px-1.5 py-0.5 text-[9px] font-bold leading-none text-white flex-shrink-0">
                  New
                </span>
              )}
