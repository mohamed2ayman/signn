"""Tests for the Qwen/OpenRouter clause-extraction provider (Steps 1-2, retry in Step 4).

Every HTTP call is MOCKED — no network, no billing. Covers: the request body (pin +
system + temperature 0), the finish_reason→stop_reason adapter, the client's BOUNDED
retry (transient → retry, exhaustion → OpenRouterError, pin on every attempt), the
_call_model routing, and the default (Anthropic) path staying untouched.
"""
import httpx
import pytest

from app.agents.base_agent import BaseAgent, ModelProvider
from app.services.openrouter_client import (
    OpenRouterError,
    _map_finish_reason,
    call_openrouter,
)

PIN = {"order": ["parasail"], "quantizations": ["fp8"], "allow_fallbacks": False}


@pytest.fixture(autouse=True)
def _no_sleep(monkeypatch):
    # Retry backoff must not actually sleep in tests.
    monkeypatch.setattr("time.sleep", lambda *a, **k: None)


def _resp(status=200, body=None, headers=None):
    req = httpx.Request("POST", "https://openrouter.ai/api/v1/chat/completions")
    return httpx.Response(status, json=body, headers=headers or {}, request=req)


def _ok(content="[]", finish="stop", cost=0.0):
    return _resp(body={
        "choices": [{"message": {"content": content}, "finish_reason": finish}],
        "usage": {"cost": cost}, "provider": "Parasail",
    })


def _patch_seq(monkeypatch, items):
    """Return/raise `items` in order (last item repeats). Tracks calls + bodies."""
    calls = {"n": 0, "bodies": []}

    def fake_post(url, headers=None, json=None, timeout=None):
        calls["n"] += 1
        calls["bodies"].append(json)
        item = items[min(calls["n"] - 1, len(items) - 1)]
        if isinstance(item, Exception):
            raise item
        return item

    monkeypatch.setattr(httpx, "post", fake_post)
    return calls


def _call(api_key="sk-test", **over):
    kw = dict(
        system="SYS-PROMPT", messages=[{"role": "user", "content": "DOC"}],
        max_tokens=28000, temperature=0, api_key=api_key,
        base_url="https://openrouter.ai/api/v1",
        model="qwen/qwen3-235b-a22b-2507", provider_pin=PIN,
    )
    kw.update(over)
    return call_openrouter(**kw)


# ── request body ────────────────────────────────────────────────────────────
def test_request_body_has_pin_system_and_temperature_zero(monkeypatch):
    calls = _patch_seq(monkeypatch, [_ok()])
    _call()
    b = calls["bodies"][0]
    assert b["provider"] == PIN and b["provider"]["allow_fallbacks"] is False
    assert b["messages"][0] == {"role": "system", "content": "SYS-PROMPT"}
    assert b["messages"][1]["content"] == "DOC"
    assert b["temperature"] == 0 and b["max_tokens"] == 28000
    assert b["usage"] == {"include": True}
    assert "sk-test" not in str(b)  # key never in the body


# ── finish_reason → stop_reason adapter ──────────────────────────────────────
def test_finish_reason_length_maps_to_max_tokens(monkeypatch):
    _patch_seq(monkeypatch, [_ok(content="[partial", finish="length", cost=0.01)])
    msg = _call().parse()
    assert msg.content[0].text == "[partial" and msg.stop_reason == "max_tokens"


def test_finish_reason_stop_maps_to_end_turn(monkeypatch):
    _patch_seq(monkeypatch, [_ok(finish="stop")])
    assert _call().parse().stop_reason == "end_turn"


def test_map_finish_reason_unit():
    assert _map_finish_reason("length") == "max_tokens"
    assert _map_finish_reason("stop") == "end_turn"
    assert _map_finish_reason("content_filter") == "content_filter"
    assert _map_finish_reason(None) is None


def test_usage_cost_captured(monkeypatch):
    _patch_seq(monkeypatch, [_ok(cost=0.0123)])
    assert _call().usage_cost == pytest.approx(0.0123)


# ── bounded retry (Step 4) ───────────────────────────────────────────────────
def test_two_429s_then_success_succeeds_after_retries(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(429), _resp(429), _ok(content="[]")])
    out = _call()
    assert out.parse().stop_reason == "end_turn"
    assert calls["n"] == 3  # retried twice then succeeded


def test_three_failures_raises_no_infinite_loop(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(429)])  # 429 forever
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 3  # BOUNDED — exactly _MAX_ATTEMPTS, not infinite


def test_pin_present_on_every_attempt(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(429), _resp(429), _ok()])
    _call()
    assert calls["n"] == 3
    for body in calls["bodies"]:
        assert body["provider"] == PIN  # pin never dropped across retries


def test_http_500_retries_then_raises(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(500, body={"error": "server"})])
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 3


def test_connection_error_retries_then_raises(monkeypatch):
    boom = httpx.ConnectError("boom", request=httpx.Request("POST", "https://x"))
    calls = _patch_seq(monkeypatch, [boom])
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 3


def test_nested_transient_429_retries_then_raises(monkeypatch):
    # HTTP 200 body carrying an upstream 429 (the exact bake-off shape).
    calls = _patch_seq(monkeypatch, [_resp(200, body={
        "error": {"message": "Provider returned error", "code": 429}})])
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 3


def test_nested_non_transient_raises_immediately(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(200, body={
        "error": {"message": "bad request", "code": 400}})])
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 1  # non-transient → no retry


def test_http_400_non_transient_raises_immediately(monkeypatch):
    calls = _patch_seq(monkeypatch, [_resp(400, body={"error": "bad"})])
    with pytest.raises(OpenRouterError):
        _call()
    assert calls["n"] == 1


def test_missing_key_raises_clear_error():
    with pytest.raises(RuntimeError) as ei:
        call_openrouter(system="S", messages=[], max_tokens=1, temperature=0,
                        api_key="", base_url="b", model="m", provider_pin=PIN)
    assert "OPENROUTER_API_KEY" in str(ei.value)


def test_openrouter_error_is_not_an_sdk_retryable(monkeypatch):
    # The existing extractor loop catches anthropic APIStatusError/APIConnectionError;
    # OpenRouterError must be NEITHER, so the loop does not double-retry it.
    from anthropic import APIConnectionError, APIStatusError
    assert not issubclass(OpenRouterError, (APIStatusError, APIConnectionError))


# ── _call_model routing ──────────────────────────────────────────────────────
def test_call_model_routes_openrouter_to_client(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        "app.services.openrouter_client.call_openrouter",
        lambda **kw: captured.update(kw) or "SENTINEL",
    )
    agent = BaseAgent()
    out = agent._call_model(
        provider=ModelProvider.OPENROUTER, system="S",
        messages=[{"role": "user", "content": "D"}], max_tokens=123,
    )
    assert out == "SENTINEL"
    assert captured["model"] == "qwen/qwen3-235b-a22b-2507"
    assert captured["provider_pin"]["allow_fallbacks"] is False
    assert captured["temperature"] == 0 and captured["max_tokens"] == 123


def test_call_model_anthropic_default_is_unchanged(monkeypatch):
    called = {"n": 0}
    monkeypatch.setattr(
        "app.services.openrouter_client.call_openrouter",
        lambda **kw: called.__setitem__("n", called["n"] + 1),
    )
    agent = BaseAgent()
    fake_msg = type("M", (), {"content": [type("B", (), {"text": "ok"})()],
                              "stop_reason": "end_turn"})()
    monkeypatch.setattr(agent._client.messages, "create", lambda **kw: fake_msg)
    out = agent._call_model(system="S", messages=[{"role": "user", "content": "D"}],
                            max_tokens=10)
    assert out is fake_msg and called["n"] == 0


def test_scrub_rejected_on_openrouter():
    agent = BaseAgent()
    with pytest.raises(ValueError):
        agent._call_model(provider=ModelProvider.OPENROUTER, system="S",
                          messages=[], max_tokens=1, scrub=True)
