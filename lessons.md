...
### Lesson #300 — A phantom Tailwind class compiles to NOTHING, and an unsized SVG then expands to fill its container
...
### Lesson #301 — A compliance finding carries no link back to the playbook position that provoked it — so don't invent one
...
### Lesson #302 — Real-Postgres specs SKIP when DATABASE_URL is absent from the process env, so a green run can be a silent no-op
