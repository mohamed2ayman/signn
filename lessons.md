...
### Lesson #297 — Tenancy family is decided by which column IS the root, not by which entities the row mentions
...
### Lesson #298 — A native host Postgres silently shadows the Docker-published port on macOS: `localhost:5432` is not the container's database
...
### Lesson #299 — `npm run lint` cannot run in backend/ at all — no general ESLint config exists in the repo
...
### Lesson #300 — Validate the MERGED result, not the patch: a PATCH that carries one half of a pair can never be checked by the DTO
