...
### Lesson #206 — A raw-QB aggregation endpoint lies to TypeScript three ways: getRawMany COUNT()s are STRINGS, grouped breakdowns are SPARSE, and wire fields can be UNDECLARED on the frontend type — verify shapes at the wire, then adapt explicitly
...
### Lesson #207 — 7.20 Slice 1 decisions: tabs are STATE (not URL routes) because no nested-route pattern exists under projects/:id, and the health formula lives as named tunable constants in a pure exported function
