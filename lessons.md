...
### Lesson #254 — Identity-TYPE is not membership-PERMISSION — "guest-ness" belongs to the BINDING, not the account (the unified-membership model), and dual user rows per email is the anti-pattern
SIGN conflated identity-TYPE (`account_type=GUEST`) with membership-PERMISSION, so a real customer invited to another org's contract was **permanently locked out**: the guest-scoped lookup keyed on `account_type=GUEST` and never found their real (MANAGING) account, and the guest-create then hit the `UQ_users_email` uniqueness → a dead-end. The industry model is unanimous (Microsoft Entra B2B / Auth0 / WorkOS / Clerk): ONE global identity + a many-to-many membership table carrying the role PER TENANT — "guest-ness" is a property of the BINDING, not of the account. The happy accident here: `guest_contract_access` WAS already exactly that membership table — the fix was to route authorization on the BINDING, not on the persona (`account_type`). The anti-pattern every source names is **dual user rows per email** (scoping email uniqueness by `account_type`): it ripples straight into login / password-reset / MFA, which all find users by BARE email and would now match two rows. **The rule:** model external access as one identity + a per-resource binding that carries the role; never fork a second user row per email to represent "the same person as a guest," and never let `account_type` be the thing that grants or denies cross-tenant access — that is the binding's job.

### Lesson #255 — When admitting a second access mode to an authz dispatch, try the EXISTING path first and fall back to the new one ONLY where the old path already denied — the common case stays provably byte-identical
Relaxing `findAccessibleContract` to admit managing-as-guest could have been written as "check binding OR org" — but that changes the common (own-org) path's query shape and makes "nothing regressed for existing users" a hope rather than a proof. Instead: **ORG-FIRST, BINDING-FALLBACK** — own-org access runs the identical `findInOrg` query FIRST (byte-identical, zero extra reads in the overwhelmingly common case), and the `guest_contract_access` binding fallback executes ONLY on the path where the old code already returned a 404. So the binding can never DOWNGRADE anyone (it only ever converts a former denial into a scoped grant), and "the common case is unchanged" is structurally true, not asserted. A leak-battery test pins it: own-org access is byte-identical to the pre-unified `findInOrg` path. **The rule:** to widen an authz gate without risking the incumbent path, sequence the old check first and gate the new mode behind "only where the old check already failed" — that makes the no-regression claim a property of the control flow, and keeps the new grant from ever weakening an existing one.

### Lesson #256 — Relax an authz gate ATOMICALLY, never piecemeal — a partial relaxation is exactly where the leak hides, and every denial must become a UNIFORM 404 (a loud 403 is an existence oracle)
`guest_contract_access` is account-type-agnostic; the ONLY things walling a managing user off the guest surface were the routing gate PLUS the per-controller `account_type === GUEST` persona asserts (upload / download / status / comments ×2 / chat). Relaxing the dispatch makes the BINDING the sole authz boundary — so if even ONE controller kept its own persona assert while the dispatch opened up, that mismatch is precisely where a cross-tenant leak (or an inconsistent 403-vs-404) hides. The fix moved ALL six asserts to a single shared `assertGuestSurfaceCaller` in ONE PR, and made every denial a UNIFORM `404` — the old loud `403` for a managing JWT with no binding was an **existence oracle** (it confirmed the contract exists). **The rule:** when you relax an authorization gate, relax every surface it guards in the SAME change and route them through ONE shared assertion — a half-migrated gate is a leak surface; and normalize all denials to an indistinguishable 404 so no error code leaks the existence of a resource the caller can't reach.

### Lesson #257 — A password-verifying path that grants cross-org access needs LOGIN-EQUAL brute-force protection — and you get it by EXTRACTING login's lockout into a shared service, never by reimplementing it
Establish-identity verifies a REAL account's password (it's how a real customer attaches a binding), but it shipped with only a per-IP throttle — strictly WEAKER than login's account-level lockout, on the exact path that hands out cross-org bindings. An attacker who could brute-force here bypasses the stronger control guarding the same credential elsewhere. The fix EXTRACTS login's inline lockout into a shared `AccountLockoutService` (5 attempts / 30-min lock, on `failed_login_attempts` + `locked_until`) that BOTH `login()` and `establishIdentity()` delegate to — so a locked account is locked EVERYWHERE, and the two paths can't drift. Extraction-not-rewrite is the load-bearing discipline: a "security fix" that quietly changes the login path's behavior is itself the bad outcome, so `login()` must be proven byte-equivalent (its existing tests stay green) while the new path gains the identical control. **The rule:** any endpoint that checks a password and grants access must carry the SAME account-level brute-force protection as login — implement it by extracting the existing control into a shared service both paths call, and prove the original path is unchanged, rather than writing a second lockout that will diverge.

### Lesson #258 — When two independently-correct changes meet at a rebase, their COMPOSITION is a new untested surface — trace the guard ordering and prove it with a test nobody asked for
Signed-state pinning (`assertContractMutable` → `CONTRACT_PINNED` 409) landed on main AFTER the unified-membership branch was cut. Each change is correct in isolation — but "do they compose?" is a separate question the rebase silently raises: the branch RELAXES the guest-surface access gate on exactly the upload path pinning now guards. The answer came from tracing the ORDER, not from assuming: persona gate → binding wall (404-first) → `assertContractMutable` (PIN, 409) → mutation. The pin sits DOWNSTREAM on the mutation path, so it fires regardless of HOW the caller authorized — a bound managing-as-guest uploading to a PINNED contract still gets 409 (the binding cannot route around the pin), while a no-binding caller still gets 404, not 409 (the pin never becomes an existence oracle). This was proven with a real-PG test nobody requested, because "both were green before the rebase" says nothing about the intersection. **The rule:** a clean rebase merges TEXT, not BEHAVIOR — when your change and a freshly-landed change touch the same control-flow surface, treat their composition as untested: trace the exact guard order, confirm the safety property holds for the new combination, and add the interaction test even though neither PR's own suite demanded it.

### Lesson #259 — Don't invent a heuristic for a distinction the data doesn't carry — surface what exists, record the limitation, and let it get smarter when the signal lands
The #8a bindings list needed a "shared by" label, and the product intent was "show the company name — but it's optional, contracts may be shared by individuals." Recon found the data CANNOT distinguish company from individual: `organizations.name` is always user-typed free text at registration (`RegisterDto.organization_name`), no placeholder is ever system-generated (the dev org "Youssef Organization" was TYPED by the user, not generated — grep proved no `'s Organization` generation code exists), and Portal Rule 8's `workspace_mode` / `is_personal` flag is unbuilt (zero hits). Any string heuristic ("detect names ending in Organization") would misclassify legitimate companies while pretending to a precision the model doesn't have. The shipped answer: no detector — emit what exists (the org name + the granter's display name, both nullable) and record the limitation explicitly in the code doc, the PR body, and CLAUDE.md, so the field can get smarter IF `workspace_mode` ever lands. **The rule:** when the data carries no signal for a distinction the product wants, surface what exists and let the consumer compose — do NOT invent a detector. Report the gap in recon (don't guess a heuristic into existence), and record the limitation where the next builder will find it, keyed to the missing signal that would unlock it.

### Lesson #260 — Send ingredients, not the cooked dish: emit un-composed atoms from the API and let the frontend compose — and normalize empty to NULL, never ""
For the #8a "shared by" label the tempting shape was ONE pre-composed server string (`shared_by`, org-name-first with a person fallback). We shipped TWO nullable fields instead — `shared_by_org` + `shared_by_user`, un-composed. A pre-composed server string DESTROYS information the UI may want (company only / person only / "Acme · shared by Sara Ahmed") and turns every presentation tweak into a backend change + redeploy; the atoms keep the presentation decision where it belongs (#8b's frontend) and cost nothing extra to emit. Corollary that makes nullable safe: the consumer MUST render gracefully when either or both are absent — so the server normalizes empty/whitespace to NULL (`labelOrNull`: trim, then null if empty), NEVER `""` (a blank label is a rendering bug waiting to happen), and NEVER a UUID fallback (tested with a UUID-regex sweep over every row of every caller). **The rule:** when an API field feeds a display label, emit the un-composed source atoms as separate nullable fields and let the frontend compose; normalize blank-ish values to NULL at the boundary so "nullable" is an honest contract, and test that no raw identifier can ever masquerade as a label.

### Lesson #261 — A zero-diff SIBLING beats a mode-switch when adding a second entry path to shipped components — "the original flow is untouched" becomes a `git diff`, not an assertion
To let a logged-in managing user open the guest viewer (which previously only consumed invitation tokens), we did NOT add a mode-branch inside `GuestViewerPage`. Instead a thin sibling `SharedContractViewerPage` reuses the same shipped guest components (`GuestLayout` / `GuestContractView` / `GuestComments` / `GuestChatPanel`), leaving `GuestViewerPage` at a literal ZERO diff. Result: "the invitation-link flow is untouched" is provable by `git diff` = 0, not asserted — an org-less guest arriving by invitation link literally cannot be affected because not one byte of their code path changed. A conditional inside `GuestViewerPage` would have made every future edit to that file a re-verification of BOTH entry modes, and turned "the token flow still works" into a claim resting on branch coverage rather than on the absence of change. **The rule:** when adding a second consumer of shipped components, prefer a sibling page/component that leaves the original byte-identical over a mode-branch threaded through it — a zero-diff on the incumbent is the strongest possible no-regression proof, and it keeps the two entry paths from sharing a surface where one's change can silently break the other.

### Lesson #262 — Valid JSON is not correct text: never round-trip non-Latin (Arabic/RTL) strings through escape encoding — automated gates cannot catch mojibake, only a human reading the language can
During the #8b i18n insert, a `unicode_escape` round-trip mojibaked the Arabic locale strings — and the file stayed VALID JSON, so it sailed through every automated check: JSON parse succeeded, key-parity matched, tsc passed, vite build passed. It would have SHIPPED broken Arabic that only a human reading Arabic would ever catch. It was caught mid-build by eyeballing the actual characters, reverted, and redone with direct UTF-8 + character-by-character verification. The lesson's teeth: the entire automated gate suite is blind to this class — "the JSON is valid" and "the JSON is correct" are different assertions, and no parser, linter, type-checker, or build step distinguishes real Arabic from plausible-looking garbage bytes. **The rule:** never pass non-Latin (Arabic / RTL / any non-ASCII) strings through `unicode_escape` or similar escape round-trips — edit them as direct UTF-8. Treat mojibake as a defect class that ONLY a human reading the target language can detect, and make sure the visual gate (the frontend EN + Arabic RTL review here) actually renders the localized strings a native reader would judge — automated green is necessary but categorically insufficient for localized text.

### Lesson #263 — When a misleading name is the right call, document the lie at the CALL SITE so the next reader doesn't "fix" a non-bug
Under Model A (the backend accepts BOTH a guest JWT and a bound managing JWT on `/guest/contracts/:id/*`), a MANAGING access token is passed to the shipped guest components through a prop named `guestJwt` — a name that LIES (it is not a guest JWT). Renaming it would churn shipped guest components we deliberately kept at minimal diff (lesson #261's whole point), so the fix was NOT to rename but to document the lie exactly where it happens: the call site in `SharedContractViewerPage` carries a clear comment explaining why a managing token flows through `guestJwt` — the component uses the value ONLY as an explicit per-request `Authorization: Bearer` on the interceptor-free `guestHttp` client, so no credential leaks onto either stack, and the binding (not the account type) is the grant. Without that comment, the next reader sees a managing token assigned to `guestJwt`, reads it as a bug, and "fixes" it — breaking the working entry path. **The rule:** when a misleading name is genuinely the right call (renaming would cost more than it's worth), neutralize the lie with a comment AT THE POINT OF USE that explains why the name is wrong and why it's still correct — put the explanation where the confused reader will actually be, not in a distant doc they won't consult before "correcting" the non-bug.

### Lesson #264 — Copying a reference across a tenant boundary creates a broken pointer — null or re-home any FK that points INTO the source tenant
Building #8d's import (copy a contract into the importer's org), the source clauses carry `source_document_id` pointing at `DocumentUpload` rows in the SOURCE org's storage — rows the importer's org cannot access. Copying that reference verbatim onto the fresh clause rows would have minted a **cross-org pointer to unreachable data**: a valid-looking FK that resolves to a row in another tenant's space. The fix set `source_document_id = NULL` on every copied clause (the only cost is cosmetic — the copy's Risk-tab doc-grouping falls back to a default group instead of a per-document one). **The rule:** when copying data across a tenant boundary, every foreign key / reference that points INTO the source tenant must be nulled or re-homed into the destination tenant — never copied verbatim. A verbatim copy of a cross-tenant reference is a dangling pointer that either leaks the existence of source-tenant rows or breaks on access; treat "does this column reference anything scoped to the source org?" as a mandatory checklist item for every field in a cross-tenant copy.

### Lesson #265 — Binding-alone authz must gate on the binding BEFORE the org-first fallback, or an own-org caller slips through with no binding
`ContractAccessService.findAccessibleContract` is **org-first, binding-fallback** (PR #164): it tries the caller's own-org read first and only consults the `guest_contract_access` binding where the org path already 404'd. That is correct for *reading* a contract — but it is the WRONG gate for a **binding-alone** decision. #8d's import must require a binding for EVERY caller, including a manager who happens to be in the source org; if I had authorized import by "did `findAccessibleContract` succeed?", an own-org manager would pass via the org path with **no binding at all**, silently violating the locked binding-alone decision. The fix asserts the binding FIRST (`assertGuestContractAccess`, uniform 404) and only THEN reads the content via `findAccessibleContract`. **The rule:** when a dispatcher has multiple success paths (org OR binding), its overall success is NOT a proof that any specific path was taken — if your policy requires one specific grant (the binding), assert that grant explicitly and independently before you lean on the multi-path helper. "The read succeeded" and "the caller holds the grant my policy requires" are different assertions; never collapse them.

### Lesson #266 — A cross-org write must be atomic and source-read-only, and its source must be the caller-scoped read — proven, not assumed
#8d's import is the first WRITE on the guest track: from a guest-scoped read of ANOTHER org's contract it creates a contract + fresh clause rows + junctions + a V1 snapshot in the IMPORTER's org. Three invariants were pinned by real-PG tests rather than trusted: **(a) atomicity** — the whole copy is ONE transaction, and a forced failure mid-copy fully rolls back (no orphan contract, no partial clause set); **(b) source read-only** — the source contract row, its clauses, junctions, and version count are byte-identical before and after (nothing in the copy path writes to the source, and no notification is sent to the source org); **(c) copy ⊆ what the caller could see** — because the copy source is the GUEST-SCOPED read (`findAccessibleContract → findForGuest`), internal-note comments, risk findings, obligations, and proposed clauses SEEDED on the source appear ZERO times in the copy. **The rule:** a feature that writes into tenant A from a read of tenant B needs all three proven by test, not argued: wrap the multi-entity write in a single transaction and prove rollback; assert the source is untouched with a full before/after state compare; and drive the copy off the same caller-scoped read the UI uses so the copy can only ever contain what that caller was already authorized to see — never a privileged read.

### Lesson #267 — Reconcile the build against the approved design EXPORT, not the spec that fed it — the input doc drifts
#8d's frontend was first built from the in-repo UI-SPEC (the design's INPUT doc), then diffed against the approved Claude Design EXPORT by rendering the export and walking every state in every language. That diff surfaced **9 real drifts a spec-build had missed**: emoji instead of the export's icon-disc SVGs; a three-band failure layout instead of the export's single unified one; paraphrased Arabic instead of the export's verbatim copy; wrong disc/spinner sizes; an approximated Import-button glyph instead of the export's exact paths. Each was a plausible, self-consistent approximation — green on every automated check — yet subtly wrong against what was actually approved. **The rule:** when a design has an approved interactive export, reconcile the build against the EXPORT, not the spec that produced it — render the export, walk each state in each language, and extract the real markup and copy. This is a cousin of #222 (design-export placeholders shipping as literal code): a spec is an approximation of the design just as a mockup placeholder is an approximation of the real component, and both ship subtly wrong unless you check against the actual approved artifact.

### Lesson #268 — Prompt caching is BILLING-ONLY — prove output-neutrality with a 0/0 cache-token control + the mechanism, and don't fear it changing the model's output
Enabling Anthropic prompt caching on the two biggest system prompts (clause_extractor + risk_analyzer) raised the natural fear: "does wrapping `system` in a `cache_control` block change what the model generates?" The answer is provably NO, and the proof is two-part. **Mechanism:** `cache_control` is a billing directive — Anthropic strips it before the prompt reaches the model, so the model sees byte-identical `system` + `messages` and generates identically (the wrapped text is the SAME text, just tagged). **Measurement:** a cache-OFF control run measured `cache_creation/cache_read = 0/0` while the cache-ON run measured them nonzero — so the ONLY observable difference between the two runs was the billing fields; token counts and output were otherwise the same. On a paid model whose output isn't bit-deterministic even at `temperature=0`, that measured 0/0-vs-nonzero control is what turns "should be neutral" into "is neutral." Two gotchas to record so nobody caches blindly: the ephemeral cache has a ~5-min TTL (only bursty callers hit it) and a minimum cacheable size (~1024 tok Sonnet / ~2048 Haiku — smaller is silently uncached), and caching a low-volume agent is net-NEGATIVE (pays the 1.25× write with no reads). **The rule:** treat prompt caching as a pure billing optimization — the cached bytes must equal the wire bytes (wrap AFTER any scrub), prove neutrality with a cache-OFF control that reads 0/0 plus the strip-before-generation mechanism, and gate it to big-prompt + bursty + high-volume paths (never blanket-enable).

### Lesson #269 — An HTTP 200 with `stop_reason == 'max_tokens'` is SILENT DATA LOSS, not an error — always inspect stop_reason, and never treat an empty/unparseable JSON parse as "complete"
Clause extraction dropped ~50% of a document's clauses on some runs with NO error surfaced: a dense-Arabic chunk's verbatim JSON output overran `max_tokens`, the API returned **HTTP 200** with `stop_reason == 'max_tokens'` and a truncated (unclosed) array, the retry loop (which only caught `APIStatusError`/`APIConnectionError`) treated it as SUCCESS, `_parse_json` failed a full `json.loads` and returned `[]`, and the whole chunk vanished — logged as a benign "returned 0 clauses." The trap is that a truncation is not an exception: it looks exactly like success at the transport layer, and an empty parse result looks exactly like "this chunk genuinely had no clauses." The fix inspects `stop_reason` (retry with more `max_tokens` headroom on `max_tokens`), adds a `raw_decode` salvage parser (keep the objects that DID parse instead of `[]`), and raises a visible `clause_extraction_incomplete` flag on ANY unparseable array — so the incompleteness is surfaced, never silent. **The rule:** after any LLM call whose output you parse, inspect `stop_reason` (or the provider's truncation signal) as a first-class outcome — a `max_tokens` finish is a partial result, not a success — and never let an empty/failed parse be indistinguishable from a legitimately-empty response; salvage what parsed and flag the gap loudly.

### Lesson #270 — A fix can introduce a WORSE bug — FIX A's sentence pieces were un-stitchable (PR #117 overlap threshold), caught by adversarial review pre-merge; raising a ceiling (B) beat restructuring (A)
The truncation fix had three layers: B (raise max_tokens), C (retry/salvage/flag), and A (PREVENTION — sentence-split an oversized boundary-less article into smaller pieces so it can't truncate). A looked obviously good, passed its own tests, and was the most "thorough." An adversarial multi-agent review of the diff proved A was a NET-NEGATIVE: its ~12k-char pieces are far larger than the PR #117 stitcher's junction-overlap threshold (~0.2 × piece length ≈ 2400 chars) while the split adds only a fixed 200-char overlap — so the pieces **cannot be rejoined**, converting a *caught+flagged* truncation into *unflagged* over-fragmentation / silent tail-drop on the exact path A targets, AND it worsened the piece count vs `main`. Meanwhile B (a one-line ceiling raise: max_tokens is a pay-for-actual ceiling, so headroom is free) fixed the root cause outright and made A redundant. A was reverted; B+C shipped. **The rule:** a change that RESTRUCTURES a pipeline can introduce a subtler failure than the one it fixes — especially where it feeds a downstream assembler (a stitcher/merger) with its own thresholds. Prefer the smallest change that removes the root cause (raise a free ceiling) over a restructure; and adversarially review a "prevention" layer against the components it feeds BEFORE merge, because it will pass its own tests while breaking the next stage's invariant.

### Lesson #271 — Non-deterministic loss at `temperature=0` (output-length variance) means ONE green run can mask a ~50% failure rate — verify with repeats, not a single pass
The clause-truncation bug was invisible for a long time because a single extraction run of the 81k fixture returned 38/38 about half the time — a green run "proved" the pipeline worked. It didn't: `temperature=0` minimizes but does NOT guarantee identical output length, so whether a dense chunk's verbatim JSON overran `max_tokens` varied run-to-run — the SAME input produced 26/38 or 38/38 on a coin-flip, and any single run could land on either. Diagnosing it required RUNNING IT 3× and watching the count swing; validating the fix required a run that showed `stop_reason=end_turn` on every chunk (not just a lucky 38/38). **The rule:** for an LLM pipeline whose failure is output-dependent (truncation, length-sensitive parsing, salient-item self-selection), a single green run is not evidence of correctness — the failure is a distribution, not a state. Repeat the run (or drive the failing condition deterministically with a mock) and inspect the CAUSE signal (`stop_reason`, token counts), not just the pass/fail of one sample; one 38/38 can hide a 50% loss rate.

### Lesson #272 — A cheaper model can match RECALL yet fail SEVERITY CALIBRATION — benchmark the decision-critical metric (verified-High recall), not raw detection
Step 3 benchmarked Haiku 4.5 vs Sonnet 4.6 on risk + compliance over the Arabic + English gold. Haiku's ALL-SEVERITY recall (does it flag the clause at all) was competitive — on Project12 it flagged 13/14 verified clauses (0.93, matching Sonnet) — so a "does it find the issues?" metric would have PASSED it. But its verified-HIGH recall (does it flag what a HUMAN confirmed is HIGH) was only **0.57 on Arabic vs Sonnet's 0.91**: Haiku detected the same clauses but rated them BELOW high. The identical pattern showed in compliance — Haiku emitted MORE findings than Sonnet yet escalated 2 vs Sonnet's 7 to CRITICAL, quietly downgrading two Egyptian Labour-Law violations. For a legal risk/compliance tool the decision-critical output is the SEVERITY ("is this HIGH/CRITICAL?"), not mere detection. **The rule:** when A/B-ing a cheaper model, score the metric the product's decision actually hinges on — here verified-High recall plus a model-vs-model head-to-head — not detection count or volume. A model can look equal on "found the issues" while being materially worse on "rated them correctly," and severity under-calibration on a safety-critical output is a rejection even at a 3× cost saving.

### Lesson #273 — An evaluation is only as clean as its inputs — confirm it scores LIVE generation against the clean EXPORTED gold, not stacked live-DB rows
Before trusting the Step-3 Haiku-vs-Sonnet numbers, we verified the benchmark's two inputs against a known prior data bug. (1) The MODEL side calls the risk agent LIVE on the gold clause TEXT (`run_stage → agent.analyze(gold_clauses)`) — it never reads stored `risk_analyses` rows. (2) The GOLD side scores the clean EXPORTED set (`docs/phase-8.3-gold/`, **0** exact-duplicate human-verified-High rows), and the scorer keys its denominator on DISTINCT `contract_clause_id` (a Python set), so even hypothetical duplicates collapse (the run reported `verified_high_clauses = 14`, not the 21/20/16 raw row counts). This mattered because the SAME three contracts still hold stacked/duplicated `risk_analyses` rows in the live DB from the pre-Issue-5 append bug (Project6/12 ≈ 97 rows across ~7 run-clusters) — had the benchmark scored those, the pre-fix duplication would have polluted the comparison. **The rule:** a new evaluation can be silently corrupted by an OLD data bug in the same tables. Explicitly trace what the benchmark READS (live-generate vs stored rows) and what it SCORES against (clean export vs live DB), and prove the scorer is duplicate-robust — don't assume an eval is clean just because its code is new.

### Lesson #274 — When you fix a bug CLASS, sweep every agent for it — compliance carried the same silent JSON-truncation as extraction (PR #177), unfixed
PR #177 fixed silent truncation in clause extraction: a `max_tokens` cut-off is an HTTP 200 with `stop_reason == 'max_tokens'` (NOT an exception), and a bare `json.loads` on the truncated text throws → the chunk's clauses are silently lost. That fix was extraction-specific (raised max_tokens tiers + a `stop_reason` retry + a `raw_decode` salvage). The Step-3 compliance benchmark then revealed the compliance agent has the IDENTICAL class, never fixed: `compliance_checker.check()` hardcodes `max_tokens = 8192` + a bare `json.loads`, so **4/6 large-Arabic compliance calls dropped EVERY finding**, nondeterministically even on English (one English response was already 8,373 output tokens, over the ceiling). It stayed invisible because compliance had never been run at scale on dense Arabic contracts with the failure actually observed. **The rule:** a silent-failure bug class (truncation, unguarded parse, missing retry) rarely lives in only one place. When you fix it in one agent, grep every sibling agent for the same pattern — `json.loads` without salvage, hardcoded `max_tokens`, no `stop_reason` inspection — and fix or file them ALL, instead of waiting for each to surface independently on its own painful path.
### Lesson #275 — A failure MARKER must be persisted OUTSIDE the transaction it aborts — mutate-then-throw inside a txn silently undoes the mutation
The 7.19 redline accept has a staleness guard: if the live clause body no longer matches the `base_content_snapshot` the author saw, the redline must flip to STALE **and** the accept must abort with 409 STALE_REDLINE — no version, no clause mutation. The naive shape (set `status = STALE` via the txn manager, then throw) is a self-defeating no-op: the throw rolls the transaction back, INCLUDING the STALE flip, so the redline silently stays PROPOSED and every retry re-runs the same doomed dance. The working shape: throw a private marker error inside the txn (rollback guarantees zero side effects — proven by asserting no version row, no `is_active` flip, no junction repoint), catch it OUTSIDE, persist the STALE flip through the regular repository as a CONDITIONAL update (`WHERE status = 'PROPOSED'`, so a racing legitimate decision is never clobbered), then surface the coded 409. **The rule:** when an abort must leave a durable trace (a status flip, an error marker, an audit row), the trace write cannot ride the transaction being aborted — route it through a separate connection/manager after the rollback, keep it conditional so it can't overwrite a concurrent winner, and test BOTH halves: the marker persisted AND everything else rolled back.

### Lesson #276 — Parallel-track migration timestamps collide silently — verify against the SHARED dev DB's migrations table, not your branch's folder
The 7.19 branch (cut from origin/main) added migration `1771000000001-CreateClauseRedlines` — the "next" timestamp after main's `1770000000004`, and the branch's migrations FOLDER confirmed it was free. Running it against the shared docker Postgres then revealed `CreateGuestSignSlips1771000000001` already applied — the parallel guest-signing track, live in the same repo, had claimed the identical timestamp from the same base. Two migrations with one timestamp make cross-branch ordering ambiguous at merge (TypeORM orders by timestamp), and neither branch can see the other in its own tree. The tell: the shared dev DB's `migrations` TABLE sees every track's applied work even when the folders don't. Fixed by bumping to `1772000000001` + deleting the stale row + re-running (the idempotent `IF NOT EXISTS` DDL made the rename safe). **The rule:** when parallel tracks are live in one repo, a migration timestamp free in YOUR tree is not free — before committing, check `SELECT name FROM migrations ORDER BY id DESC` on the shared dev DB, and on any collision take a timestamp band clear of the other track's. Idempotent DDL (IF NOT EXISTS everywhere) is what makes a late rename cheap; one more reason it is the house style.

### Lesson #277 — A conditional status-flip at the END of a txn protects the whole txn — but only for races on the SAME row; same-TARGET races need a row lock
The redline accept ends its transaction with `UPDATE clause_redlines SET status='ACCEPTED' WHERE id=$1 AND status='PROPOSED'` and throws (rolling everything back) on 0 affected rows. For two hosts accepting the SAME redline this is fully correct under READ COMMITTED: the loser blocks on the row lock, EvalPlanQual re-evaluates the predicate against the winner's committed ACCEPTED, gets 0 rows, and its ENTIRE transaction — snapshot, promoted clause, junction repoint — rolls back with the throw. At-most-once promotion falls out of putting the flip INSIDE the txn, conditional, last. What it does NOT cover: two DIFFERENT PROPOSED redlines on the SAME clause accepted truly concurrently — each flips its OWN row (both conditionals succeed), and if the second's staleness re-read ran before the first committed, both promotions land: the junction ends on the last committer and the first's promoted clause is orphaned. The Slice-1 window is tiny (the staleness re-read runs in-txn, so any accept that STARTS after the first commit dies STALE), but closing it needs `SELECT … FOR UPDATE` on the junction row before the staleness check, so the content re-read serializes behind the concurrent writer. **The rule:** a conditional flip guards races on the row it flips; when concurrent operations contend on a SHARED target row (a junction, a counter, a parent), lock the target — not just your own state row — before the read your write depends on. Filed as the known 7.19 follow-up, deliberately not built in Slice 1.

### Lesson #278 — Guest redline WRITES are gated OFF until the #8c-hardened guest surface exists — re-open ATOMICALLY WITH the hardened gate, never before
Verification of the 7.19 spine surfaced that `findAccessibleContract` would ADMIT a bound `account_type=GUEST` caller into `propose()` (the wall's guest branch grants on the binding, exactly as designed for reads) — so guest redline writes were REACHABLE through the managing surface even though the guest slice (rate limits, quotas, the #8c session posture, a guest-facing UI) doesn't exist. The fix is a STATIC write-exclusion at the service seam: `propose`/`counter` call `assertNotGuestWriter` (which reuses the dispatcher's own `isGuestUser` predicate — one identity definition, no drift) and throw the wall's exact uniform 404 (no 403, no oracle), while `list` (read) and `withdraw` (the author's own cleanup of any pre-gate row) stay open, and the `author_identity_source` enum keeps its GUEST value so the DATA MODEL stays identity-agnostic. **The rule:** when a shared access dispatcher makes a new WRITE capability reachable to a persona whose product surface isn't built, close it with an explicit static gate at the service seam — keyed on the same identity predicate the dispatcher uses, denying with the same uniform 404 — and when the persona's slice ships, re-open the gate ATOMICALLY WITH that slice's own hardening (its limits/metering/session posture), never as a separate earlier change. "The wall admits them" is an access fact, not a product decision; the gate is where the product decision lives, and it must be greppable (`assertNotGuestWriter`) so the re-open is a deliberate single-site edit.

### Lesson #279 — A second status DIMENSION gets its own column + its own guard — and workflow preconditions live INSIDE the guard, with auto-hooks riding the triggering transaction
7.19 Slice 2 needed negotiation state (DRAFT → SHARED → UNDER_REVIEW → AGREED → READY_TO_SIGN), and the tempting shape was to overload the existing lifecycle `ContractStatus` (already triple-overloaded — see the status-model-cleanup backlog). The shipped shape: a SEPARATE `negotiation_status` column with its OWN transition map + guard (`NegotiationStatusService.applyTransition` — a conditional UPDATE keyed on the from-status), and THREE rules that made it robust. (1) **No raw writes**: every mutation in the codebase routes through the one guarded method, so an illegal move — including from an AUTO-hook — surfaces as a coded 409 bug signal, never a silent write. (2) **Preconditions live inside the guard, not the controller**: `→ AGREED` requires ZERO open (PROPOSED) redlines, checked in `applyTransition` itself — a future second caller (bulk action, admin tool) cannot forget it. (3) **Auto-hooks ride the TRIGGERING action's transaction**: redline-propose's SHARED→UNDER_REVIEW move runs on the propose txn's EntityManager, so a rollback reverts BOTH (proven: forced post-hook failure → no redline row AND the lane snapped back — no orphan UNDER_REVIEW). Auto-hooks are also strictly idempotent and never move the lane backward (a re-share on UNDER_REVIEW is a no-op; READY_TO_SIGN is 7.19's terminal — the slip→pin handoff past it belongs to the #2 signing track). **The rule:** when a workflow needs state orthogonal to an existing status, give it its own column + its own single-chokepoint guard; put every business precondition inside that guard; and thread the caller's EntityManager through auto-transitions so they commit or roll back with the action that caused them.

### Lesson #280 — Knowing lesson #148 didn't prevent lesson #148: in a service mixing raw SELECT and UPDATE…RETURNING, the SELECTs' bare-rows shape camouflages the UPDATE's tuple — destructure every raw UPDATE result at the call site
The Slice-2 guard's conditional `UPDATE … RETURNING` via `manager.query` gated on `rows.length !== 1` — and every successful transition threw "Cannot transition from X to X": TypeORM 0.3 returns `[rows, rowCount]` for a raw UPDATE (the documented lesson #148), so the 2-element TUPLE failed the length check, the "lost race" branch re-read the (already-moved!) lane, and 409'd with from==to — the write applied, then the service called it a failure. The sharp part is WHY it happened despite #148 being in the lessons file: the SAME service's raw SELECTs (`currentStatus`, `openRedlineCount`) correctly return bare rows arrays, so the adjacent green patterns made the UPDATE call look right by symmetry — the inconsistency is BETWEEN statement types, not between libraries, and eyeballing the service shows four consistent-looking `manager.query` calls. The failure signature is also worth banking: a conditional-update guard that errors with **from == to** means the write SUCCEEDED and your affected-rows gate is misreading the result shape — look at the tuple, not at your transition map. **The rule:** destructure every raw TypeORM UPDATE/INSERT/DELETE result as `[rows, rowCount]` AT THE CALL SITE the moment you write it (never hold it in a `rows`-named variable), and treat "impossible self-transition" errors as a result-shape misread until proven otherwise.

### Lesson #281 — Word-level diffs are BACKEND-computed platform-wide — a new diff surface adds its diff at the API via the shared util, never a frontend diff dependency
Building the redline UI needed a base-vs-proposed word diff, and the obvious-looking move was to diff client-side next to the render. Recon of the existing version-compare pipeline showed the platform's actual shape: jsdiff (`diffWordsWithSpace`) is a BACKEND dependency only — `computeClauseDiff` (clause-diff.util) computes `wordLevelDiff` server-side, the API ships the segment stream, and the frontend's `wordDiff.ts` is purely a RENDER-layer coalesce (whole-word snapping for Arabic) feeding the shared `DiffView`. Adding a frontend jsdiff would have minted a SECOND diff algorithm whose output could drift from every other diff surface (version compare, guest proposed-version review) — subtle inconsistencies in exactly the feature whose job is showing differences. The shipped shape: the redline list response gained `word_level_diff`, computed by a SINGLE-PAIR `computeClauseDiff` call (same util, same output shape), proven by a test that asserts the payload EQUALS the util's output for the same pair plus the reconstruction invariant (non-removed segments concatenate to the proposal). The UI then reuses `DiffView` untouched — one diff algorithm, one renderer, three consumers. **The rule:** before adding a data-shaping dependency to the frontend, check which side computes that shape for the surfaces that already exist — if the platform computes it server-side, extend the API with the SAME shared util (a one-field addition) rather than introducing a client-side twin; two implementations of the same transform WILL drift, and a diff/format/normalize transform drifting between surfaces is a correctness bug users can see.

### Lesson #282 — t() fails SOFT, so missing i18n keys pass every automated gate — and a locale-PARITY check cannot catch a key missing from ALL locales equally
The Slice-3 blocker: the redline card buttons rendered raw keys ("redlines.actions.accept") because the `redlines.actions.*` group was never added to ANY locale — the component used the keys, the modal groups (`redlines.accept.*`) existed, and the two lists were close enough that eyeballing missed the gap. Every automated gate stayed green: tsc (a t() key is just a string), the component tests (the house convention mocks t() to RETURN the key, so assertions match the key either way), the build, and — the subtle one — the 3-locale PARITY script, which compares locales AGAINST EACH OTHER and therefore proves nothing about keys absent from all three equally. react-i18next fails soft (renders the key), so only a human reading the rendered UI catches it. The fix class: a COVERAGE test that extracts every t() usage from the slice's component sources (static string literals via regex + each dynamic template family expanded over its full enum domain) and asserts existence in en AND ar AND fr — plus a sanity floor on the extraction count so a broken regex can't silently pass as zero-keys-checked (a guard that scans nothing is worse than no guard). RED-verified by deleting one key: the test names the key and locale. **The rule:** for any new i18n'd surface, parity between locales is necessary but categorically insufficient — add a used-keys→locales coverage assertion, expand dynamic key families over their enums, and floor the scan count; the failure mode this closes (raw keys in production UI) is invisible to every type-level and mock-level gate you have.

### Lesson #283 — The reviewer's GATE ITEM must be observable in the review environment BEFORE merge — swap the dependency rather than promising "it works after merge"
Slice 3's explicit merge gate was Youssef SEEING RTL-correct word-level diff highlighting — yet the review environment could not show it: the shared dev backend runs `main` (bind-mounted), the highlighting depends on this PR's backend `word_level_diff` field, and the frontend's graceful fallback (full side-by-side, no highlights) made the gap look like a cosmetic afterthought ("highlights arrive after merge"). That inverts the gate: the ONE thing the human must verify pre-merge was the one thing the environment structurally couldn't display, and a graceful fallback HIDES the absence rather than flagging it. The fix was environmental, not code: stop the compose backend and run a sibling container from the same image with the BRANCH worktree bind-mounted at /app (same env-file, network alias, node_modules + uploads volumes, same port), verify the endpoint now returns the field, and hand the reviewer an environment that exhibits the gate item — revert by removing the sibling and restarting the original (other tracks' trees untouched). **The rule:** when a PR's merge condition is a human OBSERVING behavior X, ensure the review environment actually exhibits X before asking for the review — if X depends on unmerged backend code, point the review environment at the branch (a disposable sibling container beats mutating any shared tree), and treat "the fallback renders fine without it" as a warning that your gate item has silently dropped out of the review, not as reassurance.

### Lesson #284 — A post-commit notification needs a post-commit SEAM: `return txn(...)` has none, and `dispatch()` throws while `enqueueEmail()` swallows
7.19 Slice 4 hangs notifications off four redline actions, and the whole slice rests on one rule: a notification must never be able to undo a committed legal act. Two things made that harder than "wrap it in try/catch". (1) **`propose` had no post-commit seam at all** — it ended `return this.redlineRepo.manager.transaction(async (manager) => {…})`, so there is literally no statement that runs after commit; notifying inside the callback would have put an SMTP call INSIDE the transaction, where a throw rolls the proposal back and a slow mailer holds a DB connection. The fix is structural, not additive: `const saved = await txn(...)` → notify → `return saved`. The other three already had a seam (accept's post-`try/catch` log line, counter's post-`await txn`, reject's post-affected-rows guard — reject has no txn at all, its conditional UPDATE is auto-committed, so post-flip IS post-commit). (2) **The dispatch layer's error posture is ASYMMETRIC and undocumented**: `NotificationDispatchService.enqueueEmail` wraps itself in try/catch and only logs, but `dispatch()` — which writes the in-app row via `notificationsService.create` — has NO try/catch, so an FK violation on `userId`, a `title` over `varchar(500)`, or a DB blip propagates straight to the caller. Reading "the email path is best-effort" and concluding "notifications are best-effort" is exactly the wrong inference. **The rule:** before hanging a side effect off a committed action, (a) find the actual post-commit seam and create one if the method `return`s its transaction, and (b) verify the error posture of every layer you call rather than the one you read first — then own the guarantee yourself with a wrapper that cannot throw, and prove it with a test that forces the notifier to throw and then re-reads the committed row from the database.

### Lesson #285 — A column that is WRITTEN but never READ is not a usable signal — a naive read compiles, tests green, and ships English to Arabic readers
Slice 4's spec said "email copy must be in the recipient's language", and `users.preferred_language` exists (`varchar(10) NOT NULL DEFAULT 'en'`, present since InitialSchema, eager-selected, no `select:false`) — so the requirement looked already-satisfied. Recon found it had **zero read paths in the entire backend**: nothing in the platform varied on it. Worse, it is stale *by construction* — the TopBar `LanguageToggle` (the switcher users actually use) writes `localStorage` and never calls the API, while a *separate* ProfilePage dropdown writes the column but doesn't change the UI. So an Arabic-reading user very often still reads `'en'`. The trap is that `if (user.preferred_language === 'ar')` compiles, passes a test that seeds `'ar'`, and silently sends English to nearly everyone — green for the wrong reason (the #140/#180 family). Two more sharp edges: neither write path validates (`@IsString()` / `@Length(2,5)`, never `@IsIn`), so the column can hold arbitrary junk and any read MUST allowlist rather than trust; and the platform ships THREE UI locales while the copy existed in two, so `fr` needed an explicit decision (→ `en`) instead of falling through. **The rule:** before building on a stored preference, grep for its READ sites, not just its schema — a write-only column is a *plan*, not a signal. Then (a) allowlist the value with a total fallback, (b) find out which UI actually writes it and whether that matches what the user perceives, and (c) state the resulting coverage honestly in the code and the PR ("correct mechanism, frequently uninformative input") rather than letting a green test imply the feature works for everyone.

### Lesson #286 — `localhost:5432` is ambiguous when a local Postgres and Docker both bind it — a migration can report SUCCESS against the wrong database
Running the new Slice 4 migration with `DATABASE_URL=…@localhost:5432/sign_db` printed `Migration … has been executed successfully` — and `\d redline_notification_batches` in the docker container returned "Did not find any relation". Cause: TWO listeners on 5432 — a Homebrew Postgres bound to `127.0.0.1`/`[::1]`, and Docker's publish bound to `*`. `localhost` resolves to loopback, so the LOCAL (stale, months-old) instance wins every host-side connection while the container is what the app and every other developer actually use. The migration genuinely succeeded — against a database nobody reads. Nothing about the output hints at it; the only tell is checking the *target* for the object you just created. Two consequences worth banking: the fix is to address the container by an interface the local server ISN'T bound to (the machine's LAN IP, since Docker listens on `*`) or to exec inside the container, and the accidental write must be REVERTED on the stale DB so it doesn't diverge further (`migration:revert` there — which also round-trip-proves your `down()`). **The rule:** on any machine that runs both a native Postgres and a containerised one, never trust `localhost` in a `DATABASE_URL` — resolve which listener you actually hit (`lsof -nP -iTCP:5432 -sTCP:LISTEN` shows both), and VERIFY a migration by querying the created object in the intended target, never by trusting the migration runner's success line.

### Lesson #287 — Debounce with ONE row per open window: `pending_count = 0` from the UPSERT means "leading edge", and claim-by-DELETE makes the flush at-most-once
Slice 4 needed "a burst becomes one digest" without delaying the first notification. Recon ruled the shape: `@nestjs/schedule` is not installed (every cron in the repo is a Bull repeatable) and there was **zero** `{ delay: ms }` precedent — the two promising `delay:` hits were both `backoff.delay`. So rather than debut an unproven Bull feature whose state lives only in Redis, the window became a Postgres row, reusing the 5×-shipped repeatable-sweeper idiom. The shape is worth reusing: ONE row per `(contract, event_class, recipient_key)`, and a SINGLE atomic conditional UPSERT decides everything — `INSERT … pending_count 0 … ON CONFLICT DO UPDATE SET pending_count = pending_count + 1 RETURNING pending_count`, where **a returned 0 means "I just opened the window, send now" and ≥1 means "suppressed, the digest will cover it"**. No `xmax` trickery, no second query, and the row lock lives only for that statement so nothing is held across the email. The sweeper then does **claim-by-DELETE** (`DELETE … WHERE window_ends_at < NOW() … RETURNING *` in one statement): the delete IS the claim, so concurrent ticks and Bull retries cannot double-send — deliberately at-most-once, because for a courtesy notification a lost digest beats spamming a counterparty. Two details that make it safe: `recipient_key` is a synthetic `u:<uuid>` / `e:<email>` discriminator so the UNIQUE index never meets NULL-is-distinct semantics (which would silently open a second window per recipient), and the drain predicate is literally the indexed column (rows are deleted on flush, so the table only ever holds OPEN windows). One more rule, earned by adversarial review rather than by the build: **the window must RE-ARM itself when it is stale, so the primary path never depends on the sweeper.** The first cut only reset the row when the sweeper deleted it — which quietly made an *optimisation* (batching) a hard dependency of the *primary* behaviour (notifying at all): if the sweeper ever stopped — crash, paused queue, a repeatable wiped by a test, a deploy gap — the row lived forever, every later claim saw an open window, and ALL notifications for that key were suppressed permanently, with no email, no in-app row, no digest, and no error. The fix is one `CASE` inside the same UPSERT (`WHEN window_ends_at < NOW() THEN 0 … re-arm`), which degrades a dead sweeper to "digests stop, notifications keep working". **The rule:** when a background job is the only thing that clears state your foreground path reads, the foreground path must be able to clear it too — otherwise the job's availability silently becomes a correctness precondition, and its failure mode is silence rather than an error. Test it by simulating the job being *dead*, not merely slow.

**The rule:** when you need debouncing and your stack has no proven delayed-job primitive, model the window as a row and let one conditional UPSERT's RETURNING value carry the leading-edge decision; pair it with a claim-by-delete sweeper, and pick at-most-once vs at-least-once deliberately rather than inheriting it from whichever retry semantics your queue happens to have.

### Lesson #288 — If the internal audit trail records a distinction, the outward notification must too — a fixed variant per ACTION drops the qualifier that made the action meaningful
7.19 Slice 4 fired one `'accepted'` notification for every accepted redline. But `accept` supports host-side editing (`AcceptRedlineDto.editedContent`, wired end-to-end through the host UI's "modify" toggle): the promoted clause becomes the HOST's wording and is marked `ClauseReviewStatus.EDITED`. The `accepted` copy asserts "The proposed wording is now the live clause text" — so a counterparty who proposed a 5% liability cap and had it accepted-with-edits at 15% received an email stating their wording was live. The notification is their ONLY push signal about the current text of a legal document, and it said the opposite of the truth. The tell was sitting in the same file: `acceptSummary` already writes `dto.editedContent ? 'accepted with host edits' : 'accepted'` into the version-history snapshot — the code KNEW the distinction was material for the audit trail and dropped it for the human. Every automated gate stayed green because the only accept test called `accept(..., {})` with an empty DTO, so the edited path was never exercised; a passing suite says nothing about a branch no test enters. **The rule:** when you attach a notification to an action, enumerate the action's OPTIONS, not just its name — any option that changes what the recipient should believe (edited-on-accept, partial success, auto-applied defaults, superseded-by) needs its own copy variant. A quick audit: grep the same service for places that already branch on that option (audit summaries, snapshot labels, status enums); if the internal record distinguishes two outcomes and the outward message does not, the outward message is lying, and the fix is a variant plus a test that actually passes the option.

### Lesson #289 — Lock the SHARED TARGET, not your own row — and once two paths lock two rows, ONE order or you have traded a race for a deadlock
Lesson #277 named the open window; closing it taught three things the naming didn't. **(1) The lock goes on the contended row, before the read your write depends on.** Redline accept's `resolveLiveJunction` was a plain `findOne` — two accepts of two DIFFERENT redlines on one clause both read the pre-promotion body, both passed staleness, both minted a child of the same parent, and the junction `UPDATE` (which DOES serialize) landed on the last committer: a forked parent chain with the first promotion orphaned. Both conditional flips succeeded because each guarded its OWN row. The fix is `SELECT … FOR UPDATE` on the junction — the row every decider on that clause contends for — taken BEFORE the staleness read, so the loser blocks, wakes to the promoted body, and dies as the EXISTING `STALE_REDLINE`. **(2) The lock had to be a separate join-free statement**: Postgres refuses `FOR UPDATE` on the nullable side of an outer join, and TypeORM's `relations: ['clause']` emits exactly that LEFT JOIN, so `setLock('pessimistic_write')` on the hydrating read is not available — lock the junction row alone, then hydrate; correct at READ COMMITTED because EvalPlanQual re-qualifies the waiter against the winner's committed row (none of `id`/`contract_id`/`is_proposed` change under a promotion, so the lock is genuinely acquired, not silently skipped) and the follow-up read is a new statement with a fresh snapshot. **(3) The fix introduced a deadlock that the race never had.** `counter` locked redline-then-junction; the new `accept` locked junction-then-redline. An `accept(R)` racing a `counter(R)` on the SAME redline cycles → `40P01` → a 500 replacing a clean coded 409 — a strictly worse failure than the corruption's silence was loud. Fixed by moving counter's junction lock ahead of its conditional flip so ONE order holds everywhere. **Two smaller earned notes:** a status re-read under the lock makes a racing loser return the SAME code a sequential second-arriver gets, so "arrived second" and "raced and lost" are indistinguishable to the client; and the concurrency test must be RED-verified in the corruption direction (stash the fix, keep the test) — ours printed `Received length: 2` with two distinct `resulting_clause_id`s, which is the proof, whereas three of the five tests in the block passed even unfixed because they cover the same-ROW race the conditional flip already handled. **The rule:** when concurrent operations contend on a shared target, lock the target before the read your write depends on — then immediately enumerate every other path that locks the same pair of rows and force a single global order, because a two-row lock introduced on one path is a deadlock on the day a second path takes them in reverse.

### Lesson #290 — A leaky cross-validation number is not a production number — report the strict, no-leakage held-out accuracy
The clause-type bake-off first put CAMeL-BERT at **81%**, which looked competitive with Claude. That number came from a cross-validation split that leaked: clauses from the same contract (shared definitions, near-duplicate boilerplate) landed in both the train and test folds, so the model was scored partly on text it had effectively already seen. A strict held-out protocol — no contract appears on both sides — dropped the honest figure to **67%** on the same 214 human labels, which is the difference between "swap the model" and "keep Claude". **The rule:** for any model-swap decision the only number that counts is the strict no-leakage one — group-split by the real unit of correlation (here the *contract*, not the clause), and treat any headline accuracy as unverified until you know exactly how train/test were separated. A suspiciously good number is a leakage smell, not a win.

### Lesson #291 — Fine-tuning with too little data makes the model WORSE, not better — more labels is the prerequisite, not a cleverer recipe
Fine-tuning CAMeL-BERT on the ~404 available labeled clauses dropped accuracy to **43%** — well below its own 67% frozen-embedding baseline. Cause: 404 examples spread across 17 clause types starves the rare classes (several had only a handful of examples), so fine-tuning overfit the common types and destroyed the tail. The instinct "the frozen model is only mediocre, so fine-tune it" is backwards when the *dataset* is the bottleneck. **The rule:** below a real per-class floor (rule of thumb: dozens of examples per class, not a few), fine-tuning is negative-value — it trades a mediocre-but-stable model for an overfit one. Grow and balance the labeled set first; no training recipe substitutes for missing data. This is exactly why capturing the correction signal now (lesson #294) is the actual prerequisite for a future retrain.

### Lesson #292 — Don't benchmark only the extremes — test the cheap middle option
The clause-type cost question was framed as "expensive Claude vs free self-hosted models", and the self-hosted side lost (lessons #290–#291). That framing skipped the obvious middle rung: **Haiku**, which on the same 214 labels matched Sonnet (~87% overall, ties on Arabic) with zero infra to stand up. The all-or-nothing framing nearly forced the decision to "keep the premium model or accept a big accuracy drop" when a cheap, accurate, no-ops option existed the whole time. **The rule:** when optimizing cost, enumerate the FULL ladder — premium model, cheap model of the same family, self-hosted OSS, and no-op — and benchmark the cheap-managed middle before concluding you must either overpay or self-host. The middle rung frequently wins on *total* cost (infra + accuracy risk), not just headline per-token price.

### Lesson #293 — A "cheaper model" is not a saving if the task costs nothing today — check the current cost before optimizing it
Haiku matched Sonnet on clause-typing, so "switch clause-typing to Haiku" looked like a clean win. It isn't: `clause_type` is produced **inline, for free, as a byproduct of the Sonnet extraction call that already runs** — nothing is billed specifically for typing. Pulling it into a dedicated Haiku classification call would ADD a second API call (and its tokens) where the marginal cost is currently zero — a net cost INCREASE dressed as an optimization. **The rule:** before optimizing the cost of a step, measure what that step costs TODAY. If it is bundled into work you already pay for, "use a cheaper model for it" actually means "un-bundle it and pay extra". A cheaper model only saves money once the task is (or becomes) a separately-billed call — so record it as the ready option for that future, not a change to make now.

### Lesson #294 — To improve a model over time you must capture the correction signal FIRST — a write-only field that overwrites the AI value records nothing
The plan to eventually swap the clause-type model depends on a labeled dataset of real human corrections — but the app was overwriting the AI's `clause_type` in place on every edit, keeping NO record of what the AI originally said or that a human had changed it. There was literally nothing to train or evaluate a future model on. Fixing that is the real prerequisite, and it's cheap: three additive columns (`original_ai_clause_type` snapshot-once, `is_type_edited_by_user`, `clause_type_source`) turn every human edit into a stored AI-label↔human-label pair — the exact signal lesson #291 says is missing. It shipped byte-unchanged behind an inline provider default, so it captures data without changing today's behavior. **The rule:** the first step of any "improve the model later" plan is to start logging the correction signal NOW — the original prediction, a was-corrected flag, and the source — because the data you don't capture today is data you can't train on in three months. Instrument before you optimize.

### Lesson #295 — A boundary + fidelity benchmark can pass a model that fails on structure — live-test on a doc whose true shape you know
The Step-5 bake-off scored clause extraction on boundary F1 + text fidelity, and by that measure Qwen3 (parasail/fp8) tied Claude on clean Arabic (Project8: 15/15, stable across 2 runs, ~30× cheaper). But wired into the real app, Qwen promoted sub-clauses (12-1, 12-2 …) to top-level clauses — 53 where the true count is ~38 on a General Conditions doc — and nothing flagged it, because over-extraction produces clean JSON (only malformed JSON trips `clause_extraction_incomplete`). On another doc it returned 2 of ~17. The benchmark never tested sub-clause nesting, so it graded Qwen on a test that skipped the exact thing it fails. **The rule:** extraction accuracy is *right structure*, not just right boundaries + faithful text — a metric that ignores hierarchy can pass a model that gets hierarchy wrong. Before trusting a bake-off, live-test on at least one document whose correct clause count you already know, because a silent over-extraction looks identical to a clean result until someone who knows the contract counts. Verdict: Claude stays; cost never justifies a switch and Arabic accuracy did not hold.

### Lesson #296 — Identical garbage from two unrelated models means the corruption is in your input — and a clean gold on a corrupt source grades OCR-repair, not extraction
Qwen3 and Llama 4 Scout — different vendors, different endpoints — emitted byte-identical garbage on the same Arabic PDF (`Sones 2018` for `لسنة 2018`, `(4B)` for `(48)`). Models don't independently invent the same nonsense: the corruption was already in the source. Project10 is a scanned PDF mangled before any model saw it (input hashes proved all models got identical bytes). Claude "won" there only by silently reconstructing the garbage from memory — fixing `(4B)`→`(48)` in one clause but mis-guessing an advance-payment `9,015`→`9/15` (true value 15%) in another, and inconsistently across runs. So a clean gold on a corrupt source stops measuring extraction and starts measuring "who rewrites garbage best." The pipeline never caught it: the quality gate only OCR-checks empty-text PDFs and measures image blur — never whether the extracted words are real. **The rule:** identical failures from independent models point at the input, not the models — check source-vs-gold fidelity before trusting any fidelity score. Reconstruction is not extraction: a model that silently rewrites a garbled statute or percentage is guessing, which on a contract platform is worse than a visible miss.

### Lesson #297 — A safety flag that isn't displayed is functionally silent — and the fix must be reachable from the same screen as the failure
PR #177 made incomplete extraction detectable: a truncated chunk sets `clause_extraction_incomplete` and the pipeline logs "flagged for review". But a real upload dropped 9 of 38 clauses, the DB carried the flag, and the user saw NOTHING — because the banner lived inside `ProcessingStatusCard`, which `ContractDetailPage` mounted only for IN-PROGRESS docs (a `terminalStatuses` filter excluded `CLAUSES_EXTRACTED`) and mounted without `onRetry`. So the warning flashed by during processing and vanished on reload, and the "Re-run extraction" button — the remedy — was double-gated out of existence, wired only in the upload wizard. Detection had been built and verified; delivery never was. **The rule:** a quality flag is only a safety net if the user can still see it after the job finishes, on the screen where they'd look, with the remedy in reach. When you ship a detector, test the *display* path on a terminal record — not just that the flag is written. And check that the corrective action isn't gated behind the same condition as the warning: a warning you can see but can't act on is barely better than silence.

### Lesson #298 — When the user says "this worked before", believe them and verify the runtime, not just the code
The برج العرب General Conditions doc extracted 38/38 in April and June, then produced 27, 29, 36, 36 across four runs in one evening, always flagged incomplete. Two of my explanations were wrong: first "this was never fixed, only made visible" (the DB proved two clean 38/38 runs), then "a +790-char re-export shifted the chunk boundaries" (the log proved 8 chunks both then and now). A first investigation only diffed the CODE and cleared it; the user pushed back, and the second investigation checked the RUNTIME — resolved settings and their source, `.env` integrity, container drift, and the actual chunk-plan log line. Result: `CLAUSE_EXTRACT_CONCURRENCY=3` (a settings.py default, not env-driven), chunk size 15,000, max_tokens tiers 24k/40k/56k→64k, oversized article 26,759 chars → 3 pieces, `.env` containing ZERO chunking keys — i.e. identical to the working era, and untouchable by that day's .env churn. Identical config producing 27–36 on the same input is the proof: this is the FIX-A-deferred oversized-split non-determinism (#269/#270/#271), and re-runs did not recover it (4/4 flagged). **The rule:** "the code is unchanged" is not "the behaviour is unchanged" — chunking is driven by resolved settings, env files, container state, and input shape, so an investigation that stops at `git log` is incomplete. Print each setting WITH ITS SOURCE (env vs default) and compare against the log line the pipeline actually emitted. And treat a user's specific memory of their own system ("8 chunks, 3 in parallel") as evidence to test, not a claim to explain away.

### Lesson #299 — Never recursively delete a worktree that contains node_modules junctions — npm workspace symlinks point back at your real source
Cleaning up a merged worktree, a `cmd rmdir` intended to unlink three `node_modules` junctions silently failed (it launched cmd interactively instead of running the command), so the junctions were still live when `rm -rf` ran. `rm -rf` followed a junction into the main tree's `node_modules`, where npm workspaces keep symlinks (`@managex/sign` → `apps/sign`, `@managex/landing` → `apps/managex`, `@managex/tokens` → `packages/tokens`) pointing back at the real source — deleting 402 tracked source files plus `node_modules`. Everything was recoverable ONLY because all 402 were committed (`git checkout -- apps packages`) and the uncommitted WIP happened to live in different directories; `npm install` rebuilt the deps. **The rule:** remove junctions/symlinks first and VERIFY they are gone (check the exit status and re-list) before any recursive delete near a workspace root — a follow-through delete through a workspace symlink destroys source, not build state. Prefer `git worktree remove` (which handles this) over `rm -rf`, and never trust a cleanup command's silence as success. Corollary: the recovery worked because the code was committed — uncommitted work is one bad cleanup away from gone.

### Lesson #300 — Tenancy family is decided by which column IS the root, not by which entities the row mentions
7.22's `playbook_positions` mentions a contract and a project, so the reflex was to route it through the Option B contract scoped-repository (`risk → contract → project → organization_id`). Wrong family. The row carries `organization_id` DIRECTLY, and `project_id` / `contract_id` are NARROWING columns — they scope a position DOWN inside an org that already owns it, and are never consulted to establish tenancy. That makes it the `ErpConnection` class, which CLAUDE.md already names: "org-scoped, NOT contract-scoped — they carry organization_id directly, so they are outside the Option B contract chokepoint". So the wall is `loadOwned(orgId, id)` → `findOne({ id, organization_id: orgId })` → **404, never 403**, not a `*ScopedRepository`. The cheap confirmation is the lint gate: `lint:contract-repo` passes with **zero exemptions**, because the rule's enforced entity list is about rows ROOTED in the contract chain — a row that merely references a contract isn't in it. **The rule:** to pick the tenancy pattern, ask which column you would filter on if every other column were dropped. That column is the root and it dictates the family; a foreign-key mention is not membership, and reaching for the heavier chokepoint "to be safe" buys an unnecessary join and an exemption you'd have to justify.

### Lesson #301 — A native host Postgres silently shadows the Docker-published port on macOS: `localhost:5432` is not the container's database
Running a migration against `localhost:5432` reported success, and then `docker exec sign-postgres psql -c '\d playbook_positions'` said the table did not exist. Both were true. `lsof -nP -iTCP:5432 -sTCP:LISTEN` showed a native `postgres` (Postgres.app) bound to `127.0.0.1:5432` and `[::1]:5432` alongside Docker's `*:5432` — and the more specific loopback bind wins, so `localhost` reaches the HOST database while the backend container reaches the CONTAINER one via the compose hostname `postgres`. Worse, the host DB was a fully-migrated same-lineage SIGN dev DB (92 migrations), so nothing looked wrong: the migration applied cleanly, the FKs to `organizations`/`projects`/`contracts`/`users` all resolved, and the round-trip verified. Docker Desktop on macOS also does not route container IPs from the host, so `172.18.0.x:5432` just times out — there is no host-side route to the container DB while the port is shadowed. This is the same hazard `contract-parties.real-pg.spec.ts` already warns about in its header ("host Postgres.app shadows localhost:5432 — host runs are invalid"). **The rule:** never trust `localhost:5432` to be the database your app uses — verify with `lsof -nP -iTCP:5432 -sTCP:LISTEN` before believing any migration or real-PG result, and run real-PG specs IN-CONTAINER on the compose network (mount the worktree + the linux `node_modules` volume + the `.env`, and pass `DATABASE_URL=…@postgres:5432`). A migration that "worked" against the wrong DB is indistinguishable from success until something reads the table.

### Lesson #302 — `npm run lint` cannot run in backend/ at all — no general ESLint config exists in the repo
`npx eslint` on new backend files fails with "ESLint couldn't find a configuration file", and it is not a worktree artifact: `git ls-files backend | grep -i eslint` returns only `.eslintrc.contract-repo.cjs` and `tools/eslint-rules/no-bare-contract-repo-access.js`, and the main tree has no `.eslintrc*` either. The `"lint": "eslint … --fix"` script is therefore dead — which lines up with the standing CLAUDE.md note that the backend lint script isn't CI-wired (it carries `--fix`, so it would silently rewrite files in CI). The ONLY runnable lint is `npm run lint:contract-repo`, and that is the actual CI gate. **The rule:** treat `lint:contract-repo` as the backend's lint proof and don't report a green `npm run lint` you never got — if a lint command errors on missing config, prove whether it is pre-existing (`git ls-files` for the config, plus the main tree) before "fixing" it inside an unrelated feature branch, because adding a repo-wide ESLint config is its own change with its own blast radius.

### Lesson #303 — Validate the MERGED result, not the patch: a PATCH that carries one half of a pair can never be checked by the DTO
`playbook_positions` has two multi-field invariants: `rule_type` ↔ `value_config` (the jsonb is uninterpretable without its discriminator) and `scope` ↔ `project_id`/`contract_id`. Both are checkable in the CREATE DTO, where every half is present. Neither is checkable in the UPDATE DTO, because a PATCH may legally send only one half — validating there is wrong in both directions: it rejects a lone `value_config` (no `rule_type` to check it against) and it ACCEPTS a lone `rule_type` switch that orphans the stored config, e.g. flipping a RANGE row to TEXT while `{min,max,unit}` stays on disk. So the authority has to live where the stored row is visible: the service merges (`dto.x !== undefined ? dto.x : row.x`), then validates the merged triple/pair, and re-validates only what the caller actually touched so an unrelated `is_active` toggle can't fail on a row it never changed. Two supporting rules fell out. The shape check is a single shared pure function returning an error MESSAGE or null, called from BOTH the create-side class-validator constraint and the service — one authority, two entry points, no drift; and the service applies it on create too, so a direct non-HTTP caller can't bypass the ValidationPipe. The DB CHECK constraint stays as the backstop, NOT the user-facing check — reaching Postgres with an incoherent row surfaces as a 500, not a 400. **The rule:** when an invariant spans fields, decide which layer can SEE all of them for the operation in question — DTOs see the request, services see the request plus the stored row — and put the check there; splitting it across layers gives you a rule that's half-enforced in two places and fully enforced in neither.

### Lesson #304 — A char-budgeted serializer must charge for its JOIN separators and its truncation marker — and decide truncation UP FRONT, not per line
`formatAssets` in `compliance-knowledge.service.ts` caps a section at 30,000 chars by summing BLOCK lengths — but it joins blocks with `\n\n` and appends a `[...truncated — N more]` marker, and it charges for neither. With one oversized asset the overshoot is invisible (the first block trips the check and nothing is emitted); with many small assets it compounds two chars at a time. Measured: **31,166 against a 30,000 cap**. The same bug then appeared twice more in the new playbook serializer, each a different off-by-one: (1) its own marker was appended unbudgeted (442 vs a 400 cap); (2) reserving the marker per-line dropped the last item of a set that *exactly* fits and then announced a truncation that never happened; (3) charging `+1` per line while the join inserts only `n-1` separators overshot by one and dropped a position that fit. **The rule:** when writing or extending a char-capped serializer, count what you will actually emit — separators included — and reserve the truncation marker's worst-case length UP FRONT, then decide whether truncation is needed BEFORE the fill loop. Per-line marker reservation is wrong in both directions: too little and the marker never fits (short lines break with less slack than the marker needs, so it is silently never shown), too much and a set that fits loses its tail.

### Lesson #305 — When a producer and its consumer each budget against the same cap, add an ABSOLUTE backstop on the final combined string
The playbook feed composes two independently-budgeted texts into one capped section: the serializer caps its own block, then the caller subtracts that block's length and hands the remainder to the asset renderer. Both halves were "correct" by their own arithmetic and the combined result still broke the cap, because each half's rounding, separators, and truncation markers are invisible to the other. Splitting a budget across two components does not compose — it multiplies the places an off-by-one can hide. **The rule:** when a shared budget is enforced in more than one place, make exactly ONE place the guarantee: measure the final combined string and clamp it. Order the halves by priority first so the clamp removes the least important tail (here structured positions go first, so the clamp eats the lower-priority asset text). Treat the per-component budgets as an optimization that keeps the clamp from firing — never as the guarantee itself.

### Lesson #306 — Trust a RED test over a consensus review — proof beats opinion, including an adversarial panel's
An adversarial review of this slice ran five independent lenses over the diff and raised the "playbook section exceeds MAX_SECTION_CHARS" finding from FOUR of them. Every one was then REFUTED by an independent verifier pass whose explicit instruction was to default to refuted when uncertain. The finding was real: a purpose-built test measured **31,166 > 30,000** on the shipped code. The adversarial structure worked exactly as designed and still produced a false negative on a true defect — a skeptical prior plus code-reading is not measurement, and "I read it and it looks fine" loses to "I ran it and it printed 31,166". **The rule:** for any claim that can be reduced to an executable assertion, write the assertion — do not let a review verdict (yours, a panel's, or a majority's) settle it. Corollary for review-verify harnesses: a refute-by-default verifier trades false positives for false negatives, so its refutations are evidence about *plausibility*, not about *correctness*, and a refuted finding that is cheap to test should still be tested.

### Lesson #307 — A capability whose most-specific tier is inert is a half-shipped feature — land the caller thread WITH the UI that exposes it
The Slice-2 resolver implements and proves CONTRACT > PROJECT > ORG precedence, but the only production caller (`ComplianceService.runCheck` → `buildContext`) never passes `contractId`, so the CONTRACT arm is never added to the query and only PROJECT > ORG is live. Worse than a missing feature: an org that authors a contract-scoped override through the shipped Slice-1 CRUD gets the ORG default fed to the AI *as that contract's standard position*, and nothing in the response, the check row, or the logs says the override was dropped. Three review lenses found this independently; the scope-threading test passes because it drives `buildContext` directly with a `contractId` that no real caller supplies — green on a call shape production never produces (the #180/#186/#189 family again). Accepting an optional param that defaults to a safe broader scope is the right way to stage the work across slices. **The rule:** when you defer the caller thread, treat the tier as UNSHIPPED and say so in the commit and the docs — then land that one line in the SAME slice as the UI that exposes it, so you never ship a control with no effect (nor, in the other order, a capability with no control).

### Lesson #308 — A phantom Tailwind class compiles to NOTHING, and an unsized SVG then expands to fill its container
A "large semi-transparent grey magnifying glass" appeared centred over the Knowledge Base page. It survived incognito with extensions disabled, so it was served by the app — yet an earlier recon concluded "not in the codebase", because the culprit *looks* completely normal: `<svg className="absolute left-3 top-2.5 h-4.5 w-4.5 text-gray-400">`, the search icon inside the filter input. The bug is that **`4.5` is not in Tailwind's default spacing scale** (…3.5, **4, 5**…) and appears zero times in `tailwind.config.js`, so `h-4.5 w-4.5` emits no CSS at all. An `<svg>` with only a `viewBox` and no width/height has no intrinsic size, so it expands to its containing block — and because this one is `position: absolute` it is out of flow, so nothing constrains it and nothing shifts to reveal the problem. Measured in the browser with the verbatim class string: **588 × 588 px**, versus 16 × 16 for a valid `h-4 w-4`. Four sibling sites carried the same phantom class but were invisible, masked by a fixed `h-9 w-9` flex parent — latent bugs waiting for someone to change that parent. **The rule:** a class that "looks like Tailwind" is not proof it exists — off-scale values (`4.5`, `7.5`, `13`) silently produce nothing. When an element renders at an absurd size, grep the COMPILED CSS for the class before reading any component logic (`.h-4\.5` absent while `.h-4` is present is the whole diagnosis). Prefer an explicit arbitrary value (`h-[18px]`) for off-scale sizes, and remember that an unsized SVG fails loudly only when it is in flow — an absolutely-positioned one fails as a silent full-page overlay.

### Lesson #309 — A compliance finding carries no link back to the playbook position that provoked it — so don't invent one
Building the 7.22 override entry ("Adjust standard for this contract" on a PLAYBOOK-layer deviation) the obvious design is to open the popover already knowing WHICH standard position the finding violated. That link does not exist and cannot be faked: `compliance_findings` has no metadata/jsonb column (its columns are exactly layer, clause_ref, finding_type, severity, requirement, actual_text, recommendation, knowledge_asset_ref, status, …), and Slice 2 feeds the playbook to the AI as **prompt text only** — a markdown block with, deliberately, no ids — so nothing round-trips. `persistFindings` maps the AI array to a fixed 9 keys, silently discarding any extra field the model might emit. The available signals are only `layer === 'PLAYBOOK'`, the pre-existing `finding_type === 'PLAYBOOK_DEVIATION'`, and a free-text `knowledge_asset_ref` that is not a position reference. Guessing the subject from `clause_ref` or `requirement` text would be a fragile invented link that silently attaches an override to the wrong standard. The shipped design therefore has the operator pick the subject explicitly, then shows that org standard read-only. **The rule:** before designing a UI that acts *on* a specific upstream record, verify the record id actually round-trips end to end — through the prompt, through the model response, and through the persistence mapper. If the pipeline is prose in and prose out, there is no id to recover; either ask the human for the missing key or wait for a structured field, but never infer it from free text and present the inference as fact.

### Lesson #310 — Real-Postgres specs SKIP when DATABASE_URL is absent from the process env, so a green run can be a silent no-op
Verifying the 7.22 playbook backend, `npx jest --testPathPattern "playbook"` reported `127 passed` and looked like a clean pass — but the summary also said `612 skipped`, and the playbook real-PG suites were among the skips. Each of those specs opens with `const SKIP_REAL_PG = !process.env.DATABASE_URL;` and downgrades itself to a `describe.skip`, and **jest does not load `.env`** (only the Nest bootstrap does), so a `.env` sitting right there in `backend/` buys nothing. Nothing failed; the tests simply never ran, and reporting that as proof would have been a false green — the exact "passing for the wrong reason" failure mode as the ERP crypto spec that only passed while its key was unset. Exporting the URL (and rewriting the docker-internal host `@postgres:` to `@localhost:` for a host-run) made the four suites actually execute: `4 passed, 91 tests`. **The rule:** on any suite with conditional real-infra specs, read the SKIPPED count, not just the passed count, and confirm the specific suites you care about appear in the run output by name. A test that skips is not a test that passes — and a self-skipping spec is invisible in a summary line that only shows green.

### Lesson #311 — In a workspace repo, a devDependency only reaches packages that resolve FROM that workspace.
`@testing-library/user-event@14.6.1` declares `@testing-library/dom` only as a peer dep (`>=7.21.4`), so npm hoisted DTL 10.4.1 to the ROOT `node_modules`, while `@testing-library/react@14.3.1` (which depends on DTL `^9`) kept a nested 9.3.4. RTL installs its `act()` wrappers via `configure()` on its own instance; user-event reads config from the hoisted one, so every user-event dispatch updated React state outside `act()` — 370 warnings across 7 test files. Adding `"@testing-library/dom": "^9.3.4"` to **`apps/sign`** devDependencies was inert by construction: Node resolution walks UP from the requiring module, and `node_modules/@testing-library/user-event` never looks down into `apps/sign/node_modules`. It produced a third copy on disk and changed nothing — measured, 203 warnings before and 203 after. The same line in the **root** `package.json` deduped it immediately: both consumers converged on one file and suite-wide warnings fell 382 → 12. **The rule:** before choosing which `package.json` to edit, find where the CONSUMING package physically lives — `ls -d node_modules/<pkg> <workspace>/node_modules/<pkg>` — and edit the manifest it resolves from. And never accept `npm ls` as proof of a dedupe; only `require.resolve` from both consumers converging on one path, plus a measured drop in the symptom, counts as evidence.

### Lesson #312 — `apps/sign` has NO committed ESLint config, so `npm run lint` has never passed for anyone and frontend PRs are effectively unlinted
`apps/sign/package.json` defines `"lint": "eslint . --ext ts,tsx --report-unused-disable-directives --max-warnings 0"`, and the Pre-PR checklist treats a clean lint as a gate — but running it produces `ESLint couldn't find a configuration file` (ESLint 8.57.1), because **no ESLint config exists in `apps/sign` or at the repo root**: `git ls-files | grep -i eslint` returns exactly two paths, `backend/.eslintrc.contract-repo.cjs` and `backend/tools/eslint-rules/no-bare-contract-repo-access.js` — both backend-only. Confirmed on a pristine tree at the merged `main` (`git status --porcelain` empty), so it is not an artefact of any in-flight branch. The failure mode is quiet in the worst way: the script exists, CI's `Frontend Tests (SIGN)` job runs `npm test` and never invokes it, so nobody sees a red lint — the check is simply absent, and every "lint clean" claim in a frontend PR description since the config went missing was unverifiable. **The rule:** a lint script in `package.json` is not evidence that linting happens. Before citing lint as a quality gate, run it once on a clean checkout and read the output — and if CI does not invoke the script, the gate does not exist regardless of what the script says.

### Lesson #313 — Every heavy tab on ContractDetailPage is conditionally mounted, so a test harness for that page absorbs far less cross-track churn than its size suggests
`ContractDetailPage.tsx` is 2711 lines and imports a dozen heavyweight tab components, which makes it look like a test harness there would need to mock the whole platform. It does not: all thirteen tab bodies are gated on the active tab — `grep -n "activeTab === '"` returns `:1562 clauses`, `:1690 comments`, `:1871 redlines`, `:1875 risks`, `:1951 obligations`, `:1963 parties`, `:1970 guests` (`{activeTab === 'guests' && <WhoHasAccessTab contractId={contract.id} />}`), `:1973 claims`, `:1980 notices`, `:1987 subcontracts`, `:1994 compliance`, `:2003 history`, `:2014 approvals` — and the default is `useState<…>('clauses')` at `:289`. A tab component that is never selected never renders, so its data fetching never fires even when it has no `enabled` guard (`WhoHasAccessTab`'s `useQuery` has none — it fetches the instant it mounts, which is exactly why the conditional matters). Only **three** children mount unconditionally: `DocumentsNeedingReview` (`:1492`), `GuestProposedVersionsPanel` (`:1501`) and `ChatPanel` (`:2703`). Concretely: a 13-test harness covering signing and share needed eight service mocks and three component stubs, and asserts nothing about any tab — so Feature #7's tab work can land without touching it. **The rule:** before pricing test work on a large page by its line count, grep its render gates. Conditional mounting is what determines the real mount-time surface — and it also tells you which parts of the file another track can change without breaking your tests, which is the number that actually matters when several people share a hot file.

### Lesson #314 — `ContractDetailPage.tsx` has ZERO `data-testid` and exactly ONE `role` attribute in 2711 lines, so plan your query handles BEFORE writing a test against it
Measured on the current file: `grep -c "data-testid"` → **0**, `grep -c "aria-"` → **0**, and `grep -n "role="` returns a single hit — `:2676 <p … role="alert">`, the mark-signed error. Everything else must be reached by visible text or placeholder. That constraint has teeth when production is read-only: the DocuSign trigger (`:1138`), the modal heading (`:2513`) and the modal submit (`:2610`) all render the literal `Send for Signature`, so `getByRole('button', { name: 'Send for Signature' })` throws on multiple matches and the only safe handle is `getAllByRole(...)` plus an explicit `toHaveLength(2)` guard before indexing — without the length assertion, a future change to the trigger's render gate silently retargets index `[1]` at the wrong node. Similarly the share modal's `Email address` label has no `htmlFor` and does not wrap its input, so `getByLabelText` genuinely fails and `getByPlaceholderText('colleague@company.com')` is the only handle. The one bright spot is the single `role="alert"`, which makes `getByRole('alert')` unambiguous *today* and makes its absence a discriminating negative elsewhere — a property of this file's current state, not a guarantee. **The rule:** audit the query handles before writing the first assertion, because "just add a `data-testid`" is not available when the task forbids production edits. Where a literal is the only handle, mark it TEMPORARY in a comment so it gets swapped for an i18n key when that surface is localized — and never index into a multi-match query without asserting the match count first.

### Lesson #315 — `vi.clearAllMocks()` does NOT drain a `mockResolvedValueOnce` queue, so a leftover one-shot silently shifts the sequence into the next test
Proven with a throwaway probe: test A calls `svc.get.mockResolvedValueOnce('LEFTOVER_FROM_TEST_A')` and then never invokes `svc.get`; `beforeEach` runs `vi.clearAllMocks()` and re-establishes `svc.get.mockResolvedValue('DEFAULT')`; test B awaits `svc.get()` and receives **`LEFTOVER_FROM_TEST_A`** (`PROBE_RESULT=LEFTOVER_FROM_TEST_A`, test green while asserting the leak). `clearAllMocks` maps to `mockClear`, which resets `mock.calls`/`mock.results` but leaves the queued one-shot implementations intact — only `mockReset`/`resetAllMocks` drains them. The damage is worst in exactly the case the API invites: a test that needs "first call returns X, later calls return Y" queues a `...Once`, an early return or a changed await order leaves it unconsumed, and the *next* test gets a value it never configured — which usually surfaces as a confusing pass, not a failure. For sequenced returns prefer a counter-based `mockImplementation` (`let calls = 0; mock.mockImplementation(async () => (++calls === 1 ? first : later))`), which is idempotent across tests and immune to leakage regardless of which reset helper the suite uses. **The rule:** if your `beforeEach` uses `clearAllMocks`, treat every `...Once` as a live grenade with the pin out — either switch the suite to `resetAllMocks` and re-declare all defaults, or avoid `...Once` entirely. And when a mock returns something no test in the file configured, suspect a leaked one-shot before suspecting the code.

### Lesson #316 — A debounce timer stashed on the handler function object never clears, because the handler is a NEW object on every render
`ContractDetailPage.tsx:706` declares `const handleShareEmailChange = (value: string) => {…}` inside the component body, and debounces by stashing its timer on itself: `clearTimeout((handleShareEmailChange as any)._timer)` at `:715`, then `(handleShareEmailChange as any)._timer = setTimeout(async () => {… searchOrgMembers(value) …}, 300)` at `:716`. Because the arrow function is re-created on every render — and typing re-renders via `setShareEmail` — the `clearTimeout` on render *N+1* reads `_timer` off a **brand-new function object** where the property is `undefined`, cancels nothing, and the timer scheduled on render *N* fires anyway. Net effect: the control is not debounced at all; typing a 13-character email schedules ~12 live 300 ms searches instead of one. It fails silently — no exception, no visible bug, just N× the API calls — and it has a second-order hazard, since one intermediate result returning `[]` latches `shareIsInternal = false` and disables the submit. In tests this makes `user.type` non-deterministic; `await user.click(input); await user.paste(value)` fires one change event and therefore one timer, which is the difference between a stable test and a racy one. **The rule:** debounce state must live somewhere that survives re-render — `useRef`, a module-level variable, or a `useMemo`'d/`useCallback`'d handler — never on a function object re-created each render. When you see `(fn as any)._something`, check the identity of `fn` across renders before trusting that the cleanup does anything, and never assert a call count on a service behind a debounce you have not verified actually debounces.

### Lesson #317 — Comparing against display copy as control flow is a latent i18n bug: it does not throw, no test goes red, the UI just starts lying
`MySecurityPage.tsx` sets `setPwMsg('Password updated.')` on success (`:55`) and `setPwMsg(err?.response?.data?.message ?? 'Could not change password.')` on failure (`:58`), then decides the styling with an exact string comparison against the success copy: `:175 pwMsg === 'Password updated.' ? 'text-green-700' : 'text-red-600'`. Success and failure are therefore distinguished by a *sentence*, not by a status flag. Today it works only because this page is not localized (`grep -c "useTranslation"` → **0**); the moment it is — or the moment someone rewords the copy, drops the trailing period, or the backend starts returning a localized success message — a **successful** password change renders in error red. Nothing throws, no test fails, and the failure is one-directional and easy to miss in review: the error branch keeps working, because a server-supplied error message never equals the literal. **The rule:** never branch on user-visible text. Carry the outcome in a variable the copy cannot change — a boolean, a status enum, a discriminated union like `{ kind: 'success' | 'error'; message: string }` — and let the copy be purely a rendering concern. This is the same class as keying frontend logic on a backend *message* instead of its machine-readable error **code** (lesson #220): the string is presentation, and presentation is exactly the thing that is allowed to change without notice.

### Lesson #318 — A test is not proven non-vacuous until you have watched it fail; mutate the assertion, confirm red, revert
A green test proves nothing on its own — it may be asserting on an element that renders unconditionally, on a mock rather than on behaviour, or on a query that silently matches something else. The cheap, conclusive check is to mutate the assertion and confirm the failure. Concretely, on the new `ContractDetailPage` suite the most surprising claim was that the share flow has no in-flight guard, so two clicks post twice; flipping `expect(createShare).toHaveBeenCalledTimes(2)` to `1` produced `AssertionError: expected "spy" to be called 1 times, but got 2 times` and `Tests 1 failed | 12 passed`, then reverting restored 13/13. That single 30-second round trip established two things a green run could not: the assertion genuinely discriminates, **and** the double-POST is real rather than an artefact of the harness — which is what licensed reporting it as a production defect. The same technique retro-validates guards: remove a re-entry guard's release and its retry-after-failure test must go red (lesson #238), and if it does not, the test was never covering the release. **The rule:** for any assertion that encodes a surprising or load-bearing claim — especially one you are about to cite as evidence of a bug, or one that documents current-but-undesired behaviour — mutate it once, see the red, revert. Never report a defect you have only ever seen a green test agree with.

### Lesson #319 — `user.type` scrambles input on heavy components under parallel load; use `user.paste` unless per-keystroke behaviour is the thing under test
Measured in PR #222 (squash `0dfc300`) on `apps/sign/src/pages/app/ContractDetailPage.test.tsx`. `user.type` re-renders the whole 2711-line page once per character; under a loaded worker pool those keystrokes interleave and arrive scrambled — observed literally as `Jane Doe` landing as `eJ.atnees tDoe` — and the test then dies on a 5000 ms timeout. It surfaced ONLY under `npx vitest run --coverage`, where instrumentation raises the load enough to starve the pool. The fix collapsed six `user.type` calls into one paste each behind a `pasteInto` helper (`:417 await user.paste(text)`, six call sites at `:1163, :1212, :1237, :1260, :1263, :1287`, joining the pre-existing `pasteRecipient` at `:394`); the PR reports file runtime dropping **6252 ms → 4374 ms**, a 30% cut, with the failures gone. The subtlety worth keeping: the tests that FAILED were the pre-existing DocuSign ones, which still use `user.type` today (eight calls at `:609–:686`) — they were the victims of the load, never its cause, and "fixing" them would have been fixing the wrong file. Attribution was established rather than assumed: the baseline file passed 10/10 full-suite coverage runs while the new file's first version failed ~3 of 13. **The rule:** reserve `user.type` for cases where per-keystroke behaviour IS the thing under test — a debounce, an autocomplete, a keydown handler — and use `user.paste` everywhere else. And when a test flakes, ATTRIBUTE it before fixing it: stress-run the unmodified baseline first, or you will "fix" someone else's problem inside your own file.

### Lesson #320 — The thing you are looking at may not be the thing you think you are looking at
Six instances inside two weeks, across four people: a main clone checked out on a stale feature branch with uncommitted edits, mistaken for main; a diff described without naming its base ref, producing two people "correctly" disagreeing; a fix reported "already on main" from a working copy rather than from the ref; a local main several commits behind, hiding an open PR; all five docker containers bind-mounted to a working tree 42 commits and 11 days stale, so every browser verification in that window checked an old build; and — live, at the start of the session that wrote this entry — `git worktree list` reporting the main clone at `59f92ef` while `git rev-parse origin/main` returned `d3797a1`, one commit behind, in a tree carrying 28 modified or untracked paths. Every one of these is the same error wearing a different hat: a proxy for the code was trusted instead of the code. **The rule:** any claim about what the code does travels with `git rev-parse origin/main` alongside it; "I checked" means `git show origin/main:<path>`, never a file on disk; and before trusting a running environment, verify what it is actually serving, not what you assume was deployed.

### Lesson #321 — `findBy*` awaits DATA ARRIVAL, not INTERACTIVITY, so awaiting the wrong thing produces a click that is silently swallowed
In `apps/sign/src/components/guest/ImportContractModal.test.tsx` the six tests that click Import awaited the projects `<select>` — which renders as soon as the query returns rows — then clicked Confirm. But `selectedProjectId` is still `''` in that same commit (`ImportContractModal.tsx:60`), so the button is disabled (`:144`); the value is seeded one step later by a PASSIVE effect (`:74-78`). `findBy*` cannot bridge that gap: RTL's `asyncWrapper` disables the act environment for the whole poll (`pure.js:86`) and allows one macrotask of grace (`:93`), while `waitFor` resolves off a MutationObserver (`wait-for.js:95`) that fires on the commit's DOM mutation — strictly BEFORE the Scheduler task that flushes the passive effect. A click landing in that window is swallowed twice over — React suppresses `onClick` on a disabled button, and `confirmImport` early-returns on `!selectedProjectId` (`:101`) — so zero awaits happen, no request is made, and the test then burns its full budget waiting for a screen that was never coming. It reproduced on Node 20.20.2 (CI's version) and effectively never on Node 24, which is why it read as a CI-only flake. PR #226 (squash `56c25e8`) fixed it by awaiting the state the interaction actually requires — `readyConfirmButton()` at `:86-87`, `await waitFor(() => expect(screen.getByTestId('import-confirm')).toBeEnabled())`, now used at all eight call sites — and the commit message is explicit that no assertion was changed, **no timeout was raised**, no `act()` wrapper added, no skip, no console filtering. **The rule:** await the state your INTERACTION requires, not the state that merely signals data arrived — for a button, that is `toBeEnabled()`. And never fix an ordering bug with a longer timeout: the wait was never the problem, and a raised timeout only converts a fast failure into a slow one.

### Lesson #322 — A negative result is only as good as your instrument; validate it against a known defect before you report "no bug"
Asked to fix a suspected race in `ContractPartiesEditor`, a session could not reproduce it. Rather than concluding "no defect" there, it built a positive control first: it took a file with a KNOWN defect — the one PR #226 fixed — reverted that fix, and ran it under its forced-scheduler-yield harness. Three tests failed under the harness and 11/11 passed without it, which proved the harness could actually detect the class of bug being hunted. Only then did it run the suspect file, which survived 0/15. That ordering is what turned an absence of evidence into evidence of absence. **The rule:** "I couldn't reproduce it" is weak evidence and should never be reported as "there is no bug"; "my instrument reliably breaks a file that HAS this defect, and this file survives it" is strong. Build the positive control before you trust the negative — and if you cannot construct a case your instrument does catch, you have not tested the code, you have tested nothing.

### Lesson #323 — Sub-agents will launder your own scratch files back to you as independent corroboration
In one session two analysis agents cited the parent's own temporary probe files as "CORROBORATION" — including one whose header read "expected to FAIL if the race is real". That header was the parent's HYPOTHESIS, not a finding, and measurement later disproved it; but written to disk and read back by a fresh agent it was indistinguishable from evidence. The failure is structural, not a lapse in agent quality: an agent searching a tree cannot tell which files are the codebase and which are your five-minute-old notes, so anything you leave lying around comes back wearing the authority of a source. **The rule:** before treating a sub-agent's evidence as independent, check whether it originated with you. Delete scratch artifacts before dispatching analysis over the tree, keep probes outside the repo entirely, and treat any agreement that reproduces your own reasoning as a restatement rather than a confirmation — independence is a property of the SOURCE, not of the number of agents that agree.

### Lesson #324 — A status signal is not state; query the state separately
Three distinct instances. `gh pr merge --squash --delete-branch` exited 1 AFTER the merge had already landed, failing only on deleting a branch that a worktree still held — read as "the merge failed", it would have produced a duplicate merge attempt. An `npm ci` failure was masked as exit 0 because the command was piped through `tail`, and a pipeline reports the exit code of its LAST stage. And a `git worktree add` returned "user rejected" after the worktree had already been created on disk. In each case the signal and the state disagreed, and in each case trusting the signal produced a confident wrong conclusion in the opposite direction. **The rule:** never infer state from an exit code, a pipeline's status, or a rejection message. Query the state separately — `gh pr view --json state,mergedAt`, an explicitly captured `$?` on an unpiped command, `git worktree list` — and never pipe a command whose exit code you actually need through `tail` or `head`.

### Lesson #325 — Structural inference is not execution — including your own, and including when several readers agree
A session produced a structurally convincing account of a race in `ContractPartiesEditor`: empty initial state, a plain-value setter, and a hydrate effect that would overwrite an early click. Two independent analysis agents reached the same conclusion from the same reasoning. Measurement refuted all of it: React drains pending passive effects before starting a new commit, so hydrate is forced to flush first, and the click's functional updater composes rather than being discarded — measured as `count=2 names=["ServerCo",""]`. The reasoning was sound and the conclusion was wrong, which is the uncomfortable part: soundness of argument is not a proxy for truth about a runtime, and agreement between readers who share a premise is correlated error, not corroboration. **The rule:** a conclusion nobody executed is a hypothesis, no matter how many readers agree with it and no matter whose session produced it. Before reporting a race, a leak, or a defect inferred from reading, run the thing and print the value.

### Lesson #326 — Machine load and real defects have different signatures; read the signature before diagnosing
Running the full frontend suite while the docker stack, a vite preview and three worktrees' `node_modules` were live took 159 s and 221 s and produced 7 then 2 failures — every one of them `Test timed out in 5000ms`, with ZERO assertion mismatches. Quiesced, the same suite ran 15.3 s and 13.0 s with 0 failures, twice — and it was LARGER when it passed, which rules out "we removed the broken tests" as the explanation. Note the converse is not guaranteed: verifying this entry, the same 66-file / 760-test suite ran green at 1-minute load averages climbing 13 → 27 with the full docker stack up (27.2 s, and 35.4 s under `--coverage`) and green again once quiesced at load 1.96 (11.5 s) — identical content, a 2.4× wall-clock swing, zero failures at either end. So load does not deterministically produce failures; it inflates duration and raises the probability, which is exactly what makes it read as flake. **The rule:** timeouts with no assertion mismatch mean the machine, not the code; any assertion mismatch means a real defect regardless of what else was running. Quiesce and re-run before diagnosing, and never raise a timeout to make a load symptom disappear — that converts an environmental signal into a permanently slower suite.

### Lesson #327 — Assert invariants, not counts of mutable data — and know the difference between a totality guard and a bare count pin
The `party_roles` registry went 11 → 22 → 24 codes: migration `1770000000001` seeds 11, `1776000000001` adds 11 more (landed 2026-07-31, PR #211), and `1779000000001` adds `JV_CONTRACTOR` + `CONSORTIUM_MEMBER` (landed 2026-08-05, PR #235) — the last two five days apart, while a feature was being built against the set. Any test pinning that size is wrong before it is written and wrong again on the next row; `party-foundation-1a.real-pg.spec.ts:611` and `:718` now assert `toHaveLength(24)` and will go red the moment a 25th role is seeded, for no defect. But the distinction matters, because not every count assertion is the same animal. A TOTALITY GUARD asserts a map covers a named vocabulary exhaustively — `RedlinesTab.test.tsx:222` (`Object.keys(REDLINE_STATUS_BADGE).sort()).toEqual([...6 statuses])`) under the comment "Badge maps stay total — a new enum value without a badge is a compile+test failure" — and going red on a new value is the entire POINT: it forces you to add the badge. A BARE COUNT PIN asserts a number with no invariant attached — `playbookModel.test.ts:73 toHaveLength(17)`, `RiskCard.test.tsx:66 toHaveLength(17)`, `PlaybookPage.test.tsx:254 toHaveLength(19)` (which also indexes `options[18]`, so one new clause type breaks the count AND retargets the index) — and going red there teaches nothing except that the number moved. `RiskCard.test.tsx:66` is the clearest case, because the correct assertion is sitting on the very next line: `LABELS.forEach((label) => expect(screen.getByRole('button', { name: label })).toBeInTheDocument())` is the invariant, and the `toHaveLength(17)` above it adds only brittleness. **The rule:** assert the properties that must hold — every ACTIVE row renders, no INACTIVE row renders, each option sits under the group its category names, the badge map is total over the vocabulary — not the size of a set someone else owns. Keep a count only when going red is the intended alarm, and say so in a comment. A test that goes red because DATA changed teaches people that red means "ignore it", which is how a red-test discipline erodes without anyone deciding to weaken it.

### Lesson #328 — The state of the `apps/sign` frontend suite, measured 2026-08-06 at `d3797a1` — and why this belongs in a dated lesson, not in CLAUDE.md
Measured on a clean worktree off `origin/main` (`d3797a1`), Node v24.14.0, vitest 1.6.1: **66 test files / 760 tests, all passing** — 11.5 s on a quiesced machine, 27.2 s with the docker stack up (`find src -name "*.test.tsx" -o -name "*.test.ts" | wc -l` → 66, cross-checked against vitest's own `Test Files 66 passed (66)`; 706 `it`/`test` declarations expand to 760 cases via `it.each`). Suite-wide `act()` warnings are down to **12**, from the 382 that lesson #311 started with. This entry exists because a roadmap line claimed "10 files / 44 tests" — off by 6.6× on files and 17× on tests — and a stale number in a planning doc is worse than no number, because it gets quoted. Coverage (`@vitest/coverage-v8`, the only provider installed and the default here): overall **statements 36.84%** (25,977/70,511) across 284 files; the 74.15% branch figure is INFLATED and must not be quoted, because v8 only counts branches inside functions that actually executed, which is why it sits twice as high as statements. For `ContractDetailPage.tsx` — statements 68.42% (1855/2711), branches 65.37% (219/335), **functions 49.45% (45/91)** — the FUNCTION number is the honest one; note its statement denominator equals the file's line count exactly, because v8 statements here are lines, and read these from `coverage/coverage-final.json`, never the terminal table, which truncates names to strings like `...hatWindow.tsx` that collapse onto each other. Read that JSON carefully, too: 46 of 91 functions have zero hits, but a `useCallback` DECLARATION line scores a hit on every render — `:467` shows 347 while its body at `:468` shows 0 — so aggregating hits over a line range reports every handler as covered and is simply wrong (`reloadClauses`, `reloadComments`, `reloadRisks` at `:468/:478/:482` all have never-executed bodies; `handleConflictAcceptGoverning :529`, `handleConflictOverride :538`, `handleExport :676`, `handleRevokeShare :763` are zero-hit by `fnMap`). What is covered on that page: mount/load, the approval workflow, comments, clause add/remove, share, and both signing doors. What is NOT: export, conflict accept/override, revoke-share, and — the blunt version — **11 of its 13 tabs**, since the tests only ever reach `clauses` (the default) and `comments` (clicked at test `:890` and `:1200`). Page-level coverage overall is **16 of 76 page components tested, 60 untested** (admin 21, app 17, legal 13, auth 5, contractor 2, root 2) — every tested page is tested by its own co-located sibling, and the coverage report independently confirms it: exactly 16 page files have >0% and they are exactly those 16. Five tests deliberately assert CURRENT BROKEN behaviour and must be INVERTED, never deleted, when fixed — four whole tests marked `⚠️ DELIBERATE NEGATIVE TEST OF A KNOWN GAP` at `ContractDetailPage.test.tsx:672, :784, :824, :1051` (swallowed errors on signature, share and approval; share having no in-flight guard so two clicks post twice) plus one gap assertion embedded at `:1434` (`⚠️ NOTE A REAL GAP` — removing a clause has NO confirmation dialog). Finally, `npm run lint` **cannot pass**: it exits 2 with "ESLint couldn't find a configuration file", because the only two eslint files tracked in the repo are `backend/.eslintrc.contract-repo.cjs` and `backend/tools/eslint-rules/no-bare-contract-repo-access.js`, both backend-only — so no frontend PR has ever been linted (lesson #312, still true). **The rule:** a measurement belongs in a DATED lesson, not in a living architecture doc — CLAUDE.md's `### Phase 2.2 — Frontend Tests (shipped — 2026-05-10)` is a frozen shipped-record describing an 8-test greenfield, and appending today's numbers to it would manufacture exactly the stale-number problem this entry was written to correct. Re-measure before quoting any figure above; every one of them was true only on 2026-08-06 at `d3797a1`.
