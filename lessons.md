...
### Lesson #311 — Two copies of `@testing-library/dom` are installed, so every `user-event` dispatch lands OUTSIDE `act()` — the warnings are a dependency-graph defect, not sloppy tests
`apps/sign` ships **two** `@testing-library/dom` instances: `npm ls @testing-library/dom` shows `@testing-library/react@14.3.1 └── @testing-library/dom@9.3.4` (nested, because RTL 14 pins `"^9.0.0"`) alongside `@testing-library/user-event@14.6.1 └── @testing-library/dom@10.4.1` (hoisted to the root, because user-event declares DTL **only as a peer dep** — `dependencies: {}`, `peerDependencies: {"@testing-library/dom": ">=7.21.4"}` — so npm satisfies it with the newest resolvable version). RTL installs its `act()` integration by calling `configure({ asyncWrapper, eventWrapper })` on **its own** instance (`@testing-library/react/dist/pure.js:46` requires DTL, `:77` calls `configure`, wrappers at `:84`/`:105`); `require.resolve` proves the split — from RTL it resolves `…/react/node_modules/@testing-library/dom/dist/index.js`, from user-event `…/node_modules/@testing-library/dom/dist/index.js`. user-event therefore reads wrapper config from an instance where no wrapper was ever installed, so every event it dispatches updates React state outside `act()`. The signature is a perfect correlation, not a gradient: **all six** files importing `user-event` warn (PlaybookPage 111, PlaybookOverridePanel 48, ProjectDetailPage 4, NotificationsPage 2, PlaybookHouseRulesCard 1, TeamPage 1) while `fireEvent`-only files warn **zero** times — `fireEvent`/`findBy*`/`waitFor` are re-exported from RTL's wrapped instance. Isolating a purely synchronous test still warned (10×), ruling out cross-test bleed. **The rule:** when act() warnings track the event API rather than the async code, stop editing tests and run `npm ls` on the testing library — a duplicated `@testing-library/dom` cannot be fixed by `await findBy*`, `waitFor`, or wrapping in `act()`, because the unwrapped update happens *inside* the awaited dispatch. Fix the dependency graph (align RTL and user-event on one DTL major); silencing `console.error` or downgrading to `fireEvent` hides a real defect and, in the `fireEvent` case, quietly weakens what the test exercises.
