...
### Lesson #300 — A char-budgeted serializer must charge for its JOIN separators and its truncation marker — and decide truncation UP FRONT, not per line
...
### Lesson #301 — When a producer and its consumer each budget against the same cap, add an ABSOLUTE backstop on the final combined string
...
### Lesson #302 — Trust a RED test over a consensus review — proof beats opinion, including an adversarial panel's
...
### Lesson #303 — A capability whose most-specific tier is inert is a half-shipped feature — land the caller thread WITH the UI that exposes it
