...
- **BACKLOG — risk-analyzer same-language output (Arabic in → Arabic out):** the `ClauseRewriterAgent` (PR #137) already enforces same-language rewrites, but the `RiskAnalyzerAgent` still emits ENGLISH `description`/`suggestion` for Arabic contracts. Apply the same same-language rule to risk analysis. **GATE CLEARED 2026-07-14:** the gold export is captured (`docs/phase-8.3-gold/`), so regenerating risks no longer risks losing the human corrections — this can now proceed.
- **BACKLOG (post-annotation — surfaced during the gold-set build):**
  1. **Chunking-truncation fix + re-extraction** — long Arabic multi-bullet clauses lose their tails DETERMINISTICALLY per template family (NTA twins lose GC secs 10/12/15/38; Orascom family loses GC sec 3 in 4/4; O&M twins both lose clause 9); ~16+ clauses REJECTED. Root-cause the chunker + re-extract before those clauses can enter the gold set (lesson #239).
  ...
  3. **Project14 document-tab UI bug** — the app rendered 5 of 6 documents and hid a clause; investigate the document-tab render.
