...
- **TODO:** ground-truth EXPORT from the corrected rows (feeds 8.4/8.5 training) — now includes `original_recommendation`/`recommendation`; the Arabic category + swap terms + the new `riskTab.*` ar/fr labels are DRAFT `_TODO` pending Youssef's legal review; the root reversed-party EXTRACTION bug (regex in `document-processing.service.ts`) is a separate backlog fix.
- **BACKLOG — risk-analyzer same-language output (Arabic in → Arabic out):** the `ClauseRewriterAgent` (PR #137) already enforces same-language rewrites, but the `RiskAnalyzerAgent` still emits ENGLISH `description`/`suggestion` for Arabic contracts. Apply the same same-language rule to risk analysis. **GATE:** regenerating risks would WIPE the 1,061 human-corrected pre-labels — do this ONLY AFTER the gold export is captured.
